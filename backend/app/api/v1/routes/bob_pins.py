...
uses: the app owns the metadata, george_ro still does Bob's reading. See the
migration (j4k5l6m7n8o9).

That holds for Bob pinning his own answer too. The `pin_answer` tool does not
connect to anything — routes/bob.py hands the agent loop a writer bound to
the authenticated user, and it calls app.services.pin_writer.create_pin, the
same function this route calls. There is ONE write path, and it is not Bob's
to reach on his own.
...
(routes/bob_pages.py) and a pin points at it with `page_id` and holds a
...
router = APIRouter(tags=["bob-pins"])

# Pins are Bob's, so they live behind Bob's page.
_pin_user = require_page("bob")
...
async def _owned(db: AsyncSession, pin_id: uuid.UUID, user: AppUser) -> BobPin:
    """Fetch a pin the caller owns, or 404. Never 403 — see the module docstring."""
    pin = (
        await db.execute(
            select(BobPin).where(
                BobPin.id == pin_id,
                BobPin.created_by == user.username,
            )
        )
    ).scalar_one_or_none()
    if pin is None:
...
async def create_pin(
    ...
    here — Bob pins his own answers through that same function when asked in
...
async def list_pins(
    ...
    if thread_id is not None:
        ...
        rows = (
            await db.execute(
                select(BobPin)
                .where(BobPin.created_by == user.username,
                       BobPin.conversation_id.in_(ids))
                .order_by(BobPin.created_at.desc(), BobPin.id.desc())
            )
        ).scalars().all()
...
async def list_pages(
    ...
    there are any. /bob/pages is the full shape.
    """
...
async def rename_page(
    ...
    Legacy route; PATCH /bob/pages/{id} is the one by identity. The page
...
async def delete_pin(
    ...
    await db.execute(
        sa_delete(BobPin).where(
            BobPin.id == pin_id, BobPin.created_by == user.username
        )
    )
