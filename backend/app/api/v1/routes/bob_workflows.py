...
anyone with Bob access can list and run them, the creator or an admin can
...
difference from /bob/pins.

EVERY RULE LIVES IN app.services.workflow_writer, NOT HERE. Bob saves and
...
WRITES RUN ON THE APPLICATION ROLE, not on either Bob role — george_ro is
...
router = APIRouter(tags=["bob-workflows"], dependencies=[Depends(require_workflow_writes)])

# Workflows are Bob's, so they live behind Bob's page — the same gate as
# /bob/ask and /bob/pins. The finer-grained verbs are checked in the
# service against metrics.yaml (workflows.permissions).
_workflow_user = require_page("bob")
...
async def _owned(db: AsyncSession, workflow_id: uuid.UUID) -> BobWorkflow:
    """
    ...
    """
    workflow = (
        await db.execute(select(BobWorkflow).where(BobWorkflow.id == workflow_id))
    ).scalar_one_or_none()
    if workflow is None:
...
async def _with_current(db: AsyncSession, workflow: BobWorkflow) -> WorkflowOut:
    version = (
        await db.execute(
            select(BobWorkflowVersion).where(
                BobWorkflowVersion.id == workflow.current_version_id
            )
        )
    ).scalar_one_or_none()
...
async def list_workflows(
    ...
    stmt = select(BobWorkflow)
    if not include_archived:
        stmt = stmt.where(BobWorkflow.status != "archived")
    rows = (await db.execute(stmt.order_by(BobWorkflow.created_at.desc()))).scalars().all()
...
async def counts(
    ...
    total = (
        await db.execute(
            select(func.count()).select_from(BobWorkflow)
            .where(BobWorkflow.status != "archived")
        )
    ).scalar_one()
    scheduled = (
        await db.execute(
            select(func.count()).select_from(BobWorkflowSchedule)
...
async def list_versions(
    ...
    rows = (
        await db.execute(
            ...
            .where(BobWorkflowVersion.workflow_id == workflow_id)
            .order_by(BobWorkflowVersion.version.desc())
        )
    ).scalars().all()
...
async def add_schedule(
    ...
    nothing — which is what lets Bob accept "every Monday at 6" in
...
async def list_schedules(
    ...
    rows = (
        await db.execute(
            ...
            .where(BobWorkflowSchedule.workflow_id == workflow_id)
            .order_by(BobWorkflowSchedule.created_at.desc())
        )
    ).scalars().all()
...
async def update_schedule(
    ...
    schedule = (
        await db.execute(
            select(BobWorkflowSchedule).where(
                BobWorkflowSchedule.id == schedule_id,
                BobWorkflowSchedule.workflow_id == workflow_id,
            )
        )
    ).scalar_one_or_none()
    if schedule is None:
...
async def list_runs(
    ...
    rows = (
        await db.execute(
            select(BobWorkflowRun)
            .where(BobWorkflowRun.workflow_id == workflow_id)
            .order_by(BobWorkflowRun.started_at.desc())
            .limit(limit)
        )
    ).scalars().all()
...
async def get_run(
    ...
    run = (
        await db.execute(
            select(BobWorkflowRun).where(
                BobWorkflowRun.id == run_id,
                BobWorkflowRun.workflow_id == workflow_id,
            )
        )
    ).scalar_one_or_none()
    if run is None:
