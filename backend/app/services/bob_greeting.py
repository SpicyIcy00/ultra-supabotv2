...
# routes/bob.py and routes/brief.py do.
...
# What a section is called when Bob says he could not check it. Deliberately
...
#
# Bob's opening line has said "ask and I'll run it" since the greeting
...
#
# A CHIP IS A QUESTION, NOT AN ANSWER. Clicking one asks Bob in the ordinary
...
def _follow_up(row: dict, meta: dict) -> Optional[dict[str, str]]:
    ...
    if section == "newly_dead":
        ...
        return {
            ...
            # "Should we discount it" is advice, and Bob has no tool for it.
...
def follow_ups(payload: dict, defs: Optional[dict] = None) -> list[dict[str, str]]:
    ...
    (brief.notability), so the first chip belongs to the item Bob just led
    with.
...
def build_greeting(payload: dict, defs: Optional[dict] = None) -> dict[str, Any]:
    """
    Bob's opening line, built from a brief.
