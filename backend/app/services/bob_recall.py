"""
What Bob already told this person, in other chats.

THE PROBLEM THIS SOLVES, AND THE ONE IT DOES NOT. Inside a chat Bob already
...
cannot see the `bob` schema, and its log role has INSERT without SELECT, so
neither of Bob's identities can read a past conversation. The only reader is
the application role, in this process, which /bob/chats already uses.
...
untouched: every number Bob states comes from a tool result in the
...
async def recent_figures(
    ...
    Args:
        session: the application session. This reads the `bob` schema, which
            Bob's own roles cannot.
