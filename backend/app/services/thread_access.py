...
george.conversations. That made every thread Bob starts on his own (the
...
pure function the suite holds:
  ...
  2. ORG ROOT. The thread's ROOT post — the post whose id IS the thread id,
     which is how river_writer names Bob's own posts — was written by
     Bob and is org-visible. A company-level post is something anyone in
...
by the replier, private by default (bob_post.default_visibility), and the
answer Bob gives is theirs too. Other people keep seeing the brief; they
...
# thread, exactly as routes/bob.py reads it.
...
# to a thread (its tool calls, its pins) is joined by them. routes/bob.py's
...
async def conversations_in_thread(
    ...
    EMPTY IS A REAL ANSWER and is not an error: a thread Bob started on his
