...
TWO WRITERS, ONE TABLE, AND NEITHER IS BOB'S READ ROLE.
...
VISIBILITY IS PER POST. Bob's own posts are 'org' because a brief that fires
...
#   answer            Bob answering something
...
#   pin_confirmation  a pin Bob made because he was asked to
#   system            the app speaking about itself
POST_KINDS = (
    "brief", "notice", "answer", "question",
    "approval", "workflow_run", "pin_confirmation", "system",
    # A watch that fired. Org, like everything Bob initiates — and unlike
    # the other Bob kinds, most days it writes nothing at all.
    "watch",
)

POST_AUTHORS = ("bob", "user")
POST_VISIBILITY = ("org", "private")

# Which kinds Bob writes. He authors all of these; who they BELONG to is a
...
# Bob's kinds that are nonetheless somebody's private business.
#
...
class BobPost(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("author IN ('bob', 'user')", name="ck_posts_author"),
    ...
    #: 'bob' or 'user' — which side of the thread this is drawn on. Not a
    #: user id: Bob has no account.
    ...
    #: NULL for Bob's own posts, because Bob has no account.
    ...
    #: person who asked, not Bob. Those two facts were one column until
...
def default_visibility(kind: str) -> str:
    ...
    Bob's own posts are org-level BECAUSE OF WHAT THEY ARE, not because he
