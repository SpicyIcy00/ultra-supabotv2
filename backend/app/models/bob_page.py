...
that the thread bound to it, the URL that opens it, the reader Bob reads it
...
under the title, it is editable, and when Bob reads the page it is handed to
...
remove, move, place, delete — whoever made them. A button and Bob's injected
...
class BobPageEvent(Base):
    __tablename__ = "page_events"
    __table_args__ = (
        CheckConstraint("actor IN ('user', 'bob')", name="ck_page_events_actor"),
