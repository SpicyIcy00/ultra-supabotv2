...
def test_the_stale_notice_is_built_that_way_at_all(tmp_path):
    ...
    protects nothing. This one always runs: it reads the source.
    """
    src = (ROOT / "tools" / "brief.py").read_text(encoding="utf-8")
    block = src[src.index('"kind": "stale_sources"'):]
    block = block[:block.index('"source": "metrics.yaml: brief.freshness"')]
    message = block[block.index('"message"'):block.index('"guidance"')]
    guidance = block[block.index('"guidance"'):]

    # The reader's line is built from the yaml's reader_names, never the raw key.
    assert "reader_names" in src
    assert "_req(defs, \"brief.freshness.reader_names\")" in src
    assert "s['source']" not in message, "the message interpolates the table name again"
    # And the per-source dates -- the ~100 words that reached his answer -- are
    # in guidance, which the contract says is never rendered.
    assert "s['latest']" in guidance and "s['age_days']" in guidance
    assert "frozen" in guidance
