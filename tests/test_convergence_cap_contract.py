...
def test_a_compose_that_names_the_claim_and_says_nothing_settles_on_the_claim(monkeypatch):
    """
    ...
    round — 53.9 s, 24.6 s and 26.6 s of the 156 the owner waited. That got a
    reminder, once. REVERSED 2026-09-22 (finishing W1.3; checks fix, they do
    not argue): the claim is his own words, so the round settles on it and
    code puts it where the answer goes — no reminder, no further round. It was
    the one round standing between a broad answer and two rounds.
    """
    writes: list = []
    frames, requests = _drive(monkeypatch, [
        _reads(2),
    ...
    assert not [t for t in said if "Write the answer NOW" in t], said
    assert len(requests) == 2, "a round was bought to write what the claim already says"
    composed = [f for f in frames_of(frames, "compose") if not f.get("default")]
    assert composed and "a" in [b["key"] for b in composed[-1]["blocks"]]
    assert frames_of(frames, "done")[0]["status"] == "ok"
