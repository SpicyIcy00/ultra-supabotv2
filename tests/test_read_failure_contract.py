...
WHAT WENT WRONG, twice, in one morning:
  ...
  2. "that monday was a holiday" — a good answer, then
     ...
     Every prior George turn in the seeded history ended with that marker,
...
def test_a_query_timeout_returns_a_refusal_in_words(monkeypatch):
    monkeypatch.setitem(george_loop.TOOL_FUNCTIONS, "get_sales", _tool_that_times_out)
    payload, err, _ms = asyncio.run(george_loop._call_tool("get_sales", {}))
    ...
    assert RAW in payload[george_loop.DIAGNOSTIC_KEY]


def test_any_other_database_error_is_a_refusal_too(monkeypatch):
    monkeypatch.setitem(george_loop.TOOL_FUNCTIONS, "get_sales", _tool_that_breaks)
    payload, err, _ms = asyncio.run(george_loop._call_tool("get_sales", {}))
    assert err == " ".join(str(req(DEFS, "failures.reads.database")).split())
    assert "UndefinedTable" in payload[george_loop.DIAGNOSTIC_KEY]
...
def test_the_three_refusal_classes_still_come_back_the_same_way(monkeypatch):
    ...
    monkeypatch.setitem(george_loop.TOOL_FUNCTIONS, "get_sales", refuses)
    _payload, err, _ms = asyncio.run(george_loop._call_tool("get_sales", {}))
...
def test_a_timed_out_read_reaches_the_model_as_words_and_the_turn_still_answers(monkeypatch):
    ...
    """
    monkeypatch.setitem(george_loop.TOOL_FUNCTIONS, "get_purchase_plan", _tool_that_times_out)
    fake = FakeClient([
    ...
    async def collect():
        return [f async for f in george_loop.run("make an ordering system from Judy")]
...
def test_an_unhandled_exception_is_told_in_words_and_logged_raw(monkeypatch):
    monkeypatch.setattr(george_loop.anthropic, "AsyncAnthropic", lambda *a, **k: _ClientThatBreaks())
    StubLog.instances.clear()
    monkeypatch.setattr(george_loop, "ConversationLog", StubLog)
...
def test_the_call_list_opens_the_turn_after_the_answer_not_the_answer_itself():
    messages = george_loop._seed_history([
        ...
        {"role": "george", "text": "OPUS led.", "tool_calls": [SALES]},
        {"role": "user", "text": "why?", "tool_calls": []},
        {"role": "george", "text": "Footfall.", "tool_calls": []},
    ], {})
    assert [m["role"] for m in messages] == ["user", "assistant", "user", "assistant"]
    assert messages[1]["content"] == "OPUS led."
    assert messages[2]["content"].startswith(george_loop.HISTORY_MARKER)
...
def test_an_answer_with_no_turn_after_it_keeps_its_list():
    ...
    messages = george_loop._seed_history([
        ...
        {"role": "george", "text": "a", "tool_calls": [SALES]},
    ], {})
    assert george_loop.HISTORY_MARKER in messages[-1]["content"]
...
def test_an_echoed_call_list_is_stripped_from_the_answer():
    ...
    answer, echoed = george_loop._strip_history_marker(said)
    ...
    answer, echoed = george_loop._strip_history_marker("Fine.\n\n[Calls behind this answer: compose({")
    assert echoed and answer == "Fine."
    # And an answer with none is untouched.
    assert george_loop._strip_history_marker("Fine.") == ("Fine.", False)
...
def test_the_echo_is_stripped_in_the_loop_and_recorded(monkeypatch):
    ...
    async def collect():
        return [f async for f in george_loop.run("anything?")]
    frames = asyncio.run(collect())
    text = "\n".join(frames)
