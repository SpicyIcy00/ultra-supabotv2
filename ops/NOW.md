...
| Next card | **P2.g — the estate switch.** **`ops/DOGFOOD_LOG.md` Open is still EMPTY**, so the gate's five-day count is on day one from 2026-09-15. **P2.f closed and shipped the same session, and its migration `x8y9z0a1b2c3` RAN** — `/health` reports it current and expected on `c87fda56`, and the swap cost two 502s over 66 s. That was the first migration since P1.h and it had been rehearsed on no machine, because there is none here to rehearse it on: no local Postgres, no Docker, and `ops/local_postgres.py` expects PostgreSQL binaries under `verification/postgres/pgsql/` that the checkout does not carry (`FileNotFoundError` on `init`). **A future card with a migration ships the same way and should watch `/health` through the swap from before the push**, which is what made this one's latency a measurement rather than a bound. **`SYSTEM_PROMPT` is 1,791 words, sha `ee1d17d1`** — a session quoting `fa166e19` is quoting a prompt that no longer ships, and the `record_belief` tool schema changed with it. **Nobody has opened a browser on any of the last seven cards**, which is the standing shortfall and is bigger than any card in the file: P2.a, P2.b, P2.c, P2.l, P2.d, P2.e and P2.f are all held by test processes alone. |
