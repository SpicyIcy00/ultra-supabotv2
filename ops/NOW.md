...
| Next card | **P2.g — the estate switch.** **`ops/DOGFOOD_LOG.md` Open is still EMPTY**, so the gate's five-day count is on day one from 2026-09-15. **P2.f closed the same session and it carries a migration** — `x8y9z0a1b2c3`, the first since `w7x8y9z0a1b2` and the first on any card since P1.h. **IT HAS BEEN RUN NOWHERE.** There is no local Postgres on this machine and no Docker, and `ops/local_postgres.py` expects PostgreSQL binaries under `verification/postgres/pgsql/` that the checkout does not carry — it fails `FileNotFoundError` on `init`. So the first machine to execute that DDL is Railway, at deploy. That is loud rather than silent (the app refuses to serve against a schema behind head, which is what `schema_check` is for), but a session pushing this should **watch `/health` through the swap and read the schema it reports**, not assume it. The grounding constraint is `NOT VALID` deliberately so an old belief row cannot fail the upgrade. **`SYSTEM_PROMPT` moved for the first time since P2.c**: 1,799 → 1,791 words, sha `fa166e19` → `ee1d17d1`, and the tool schemas changed with it — a session quoting the old sha is quoting a prompt that no longer ships. **Nobody has opened a browser on any of the last seven cards**, which is the standing shortfall and is bigger than any card in the file: P2.a, P2.b, P2.c, P2.l, P2.d, P2.e and P2.f are all held by test processes alone. |
...
- [x] **P2.f what do you remember?** — CLOSED 2026-09-15. **`view_memory` is
      ...
      **THE SHORTFALL, AND THE FIRST ONE IS NEW.** **The migration has been
      run NOWHERE** — there is no local Postgres on this machine and
      `ops/local_postgres.py` needs binaries that are not in the checkout, so
      the DDL is unrehearsed and the first machine to run it is Railway. A
      failed `alembic upgrade head` fails the deploy loudly rather than
      silently, which is the design, but it would be George down until
      somebody looked. **The done-when is not held by anything that ran.**
