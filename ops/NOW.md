...
| Head | **`e16573f` — UNPUSHED.** Two commits from P1.e: the six marks (`369d0e8`) and both copies of the plan (`e16573f`). **No migration** — the change is the renderer, the eval suite and the notes. Behind them, `b0ccfba` is what is live (see the row below). Before that, `fb303ba` was pushed and live 2026-09-14 with five commits: P1.c (`cf5806e`), the plan page moving into the repo with its contract test, and two eval fixes; and `a3003ef` on 2026-09-13 carried 32 commits that had sat unpushed: P1.b's default composition, the v2 voice suite (`bfb168f`), the $0 replay (`235d236`) and P1.g (`c508965`). |
