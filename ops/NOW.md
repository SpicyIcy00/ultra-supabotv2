...
`python` cannot import the backend (pinned SQLAlchemy).
    ...
    .venv\Scripts\python.exe ops/verify_integration.py pure     # 1,990 here (P2S.11); CI not re-counted
...
with each other, across builds with `--since`.        # the weekly sweep
    ...
    cd frontend && npx vitest run                               # 957 expected (P2S.10, 2026-09-18)
...
(the link the platform returned on the 2026-09-18 republish, version 72 after P2S.11). **Since
