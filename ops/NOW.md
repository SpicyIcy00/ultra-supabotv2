...
| Head | **UNPUSHED since `1edf8fb`, so the head and the live build are not the same thing.** `0daa0d6` is the dogfood fix — a caveat may carry a figure a read returned — and the commits above it are this file and the log saying so. **No migration.** What is actually live is the row below and `/health`: **`1edf8fb` — pushed and live 2026-09-14.** Two commits: **P1.h** — effort per turn on a mid-conversation system message, and the volunteering and restatement gates deleting instead of buying a second answer (`5d6c250`) — and a dogfood entry a concurrent session wrote about the caveat slot's no-digits rule (`1edf8fb`), which is the same thing P1.h's run found eight times. **No migration.** Before it, `079359f`; before that, `8ba0080`, `91dec69` and `b0ccfba`. |
| Live | **`1edf8fb`**, confirmed from `/health`: healthy, schema `w7x8y9z0a1b2` current and expected, deployment `666da8f4`, **no migration**. **THE SWAP ITSELF WAS NOT OBSERVED AND THIS ROW WILL NOT SAY IT WAS CLEAN.** Polling started after the push and the FIRST poll already had the new build, so the transition happened inside the gap: 24 polls every 20 s from 18:26:03 to 18:33:51, every one 200 and every one `1edf8fbc`. What is evidenced is that the new build is up and settled, not that nothing blipped on the way — the last two swaps each cost one 502 at exactly that moment, so the gap is where the interesting second would have been. Start the poll BEFORE the push next time. Read `/health` rather than believing this row. |
| Last deploy | `079359f`, and before it `8ba0080` — both live and healthy when recorded, and each swap cost **one 502** at the moment of the swap with 200s either side. The row that ended the run of five clean swaps is `91dec69`; three in a row have now blipped once. None of the three carried a migration, so it is not the migration path they live in — which is the one thing the 50-minute outage on `8b0325a` IS known to live in. That outage was migration `w7x8y9z0a1b2` applied on `8b0325a`: the first boots crashlooped, the migration did not apply, nothing was readable from outside, and it came up on a later retry. Root cause still unknown — the Railway deploy log for that build has not been read. `69b51bd` is the fix for the *invisibility*, not for the cause. |
