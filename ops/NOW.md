...
- [ ] **P3.h StoreHub exports in — the API, or the upload page** — FIRST,
...
three a dogfood fix: **six open cards at four a week is a week and a half of
...
`python` cannot import the backend (pinned SQLAlchemy).
    ...
    .venv\Scripts\python.exe ops/verify_integration.py pure     # 2,028 here (the speed fix, 2026-09-19); CI not re-counted
...
with each other, across builds with `--since`.        # the weekly sweep
    ...
    cd frontend && npx vitest run                               # 986 expected (P2S.✓, 2026-09-18)
