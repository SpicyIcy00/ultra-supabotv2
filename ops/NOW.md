...
`python` cannot import the backend (pinned SQLAlchemy).
    ...
    .venv\Scripts\python.exe ops/verify_integration.py pure     # 2,015 here (P2S.5); CI not re-counted
...
with each other, across builds with `--since`.        # the weekly sweep
    ...
    cd frontend && npx vitest run                               # 986 expected (P2S.5, 2026-09-18)
