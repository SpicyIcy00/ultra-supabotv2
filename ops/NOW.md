...
**Read it BEFORE the push and poll until it changes** — the same discipline as the Railway watch, and the reason those swaps are measured and these have never been. It is not a sha and maps to no commit; it says only that the built content differs. `/health` on Railway confirms the BACKEND only, and a session reporting a frontend fix as live off it is reporting the wrong half.
