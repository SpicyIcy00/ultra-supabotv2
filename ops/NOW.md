...
**Wave 1 is merged and measured (2026-09-22); wave 2 is next.** Five agents, five
worktrees, merged in order with every suite after each merge (DECISIONS, same
date, has what each card decided). Suites: pure **2,349** passed, 0 skipped (was
2,215); frontend **1,233/1,233** (was 1,207 with 2 clock-dependent failures, fixed
by W1.1); `tsc -b --force` clean.

| measure | before | after wave 1 | target |
|---|---|---|---|
| capability test, median turn | 108 s | **17.7 s** (21 turns) | — |
| a fact, 5 runs | 10–25 s | **6.5 s** median (3.6–10.6) | 5–15 s — met |
| a narrow question, 5 runs | ~150 s inflated | **53.9 s** median (21–58), no page | 20–40 s — **missed** |
| a broad question, 5 runs | 180–257 s | **148.8 s** median (127–203); drawable overview: **136.0 s** (129–159) | one read, two rounds — **1 of 5** |
| eval suite on DeepSeek, first run | never run | 20 passed, 26 failed, 3 skipped, 1 xfailed | — |

**The capability test after wave 1**, same phrases: "Keep this" pins; "I want this
every Monday" schedules the report, not the watch; the workflow saves and its
Monday slot delivers to the room; "Use 30-day velocity" makes version 2 of the
same plan; "Remember that…" is 5.1 s and one line; "Handle the AJI BARN reorder"
returns a draft of 23 moves and 9 orders. Still missing, as planned: thresholds
and managers' requests (W2.2). Nothing it made was switched on; all 23 rows were
deleted.

**The 26 failing evals are the baseline, not the wave.** Every failing case was
run on the pre-wave commit too, and all but three fail there: the investigation
cases' call caps (written for Opus), the seven page-workshop cases, the morning,
`analyze`. Of the three, `gate_1` failed because the check read only his claim
(now it reads every slot the room draws); `no_rows_is_said_as_no_sales` passed on
a re-run and `thread_3_correction` on two. The call-cap and page-workshop evals need rewriting for DeepSeek's shape
before they measure anything — not scheduled on a card.

**Found by the wave, not fixed:** told "you don't need my approval under
₱20,000", Bob said *"I key orders under his line myself"* — an action he cannot
take (W2.2's). The kept-page date filter needs an `edit_page` operation that no
card owns yet.
