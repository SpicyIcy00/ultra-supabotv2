...
| Head | **`ed0a5eb` — committed 2026-09-14, NOT PUSHED.** P1.i: `POST /george/replay` takes a stored call named rather than sent, changes one scope argument, records the change on the answer post, and returns a board frame. **No migration** — the record is a key inside `posts.payload`. Before it, `fda8bad` and `ab01579`, which is what is live. |
