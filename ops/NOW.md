...
| Next card | **P2.f — what do you remember?** **`ops/DOGFOOD_LOG.md` Open is still EMPTY**, so the gate's five-day count is on day one from 2026-09-15. **THE FRONTEND was red on origin for the whole of P2.e**: P2.d left `ComposerProps` declaring `subjects` twice and `tsc -b` refused it, so the **Vercel** build failed and the room has not updated since `8037259`. **The backend was never red** — `/health` was serving `7cb1ea5f` before this push, so Railway deployed P2.d fine. A session that says "the build failed" says WHICH; the two platforms fail independently and only one of them runs `tsc`. The runtime half was worse than the build half — in JSX the later attribute wins, so Room.tsx passing both meant the picked subjects never reached the composer and every chip above the line would have drawn `undefined ×`. Fixed in `1043919` as its own commit, ahead of the card, with two tests holding the pair apart. **Pushed 2026-09-15 with P2.e on the owner's word**, so the Vercel build should be green again — **nobody has confirmed that from a terminal**, because no session has ever read a Vercel build status and `/health` cannot see one. A hard refresh of the room is the check. **The command that missed it three times is `npx tsc --noEmit -p tsconfig.json`** — `frontend/tsconfig.json` is `{"files": [], "references": [...]}`, so it exits 0 having compiled nothing. **The typecheck in §4 is now `npx tsc -b --force`**; plain `-b` can report clean off a stale `.tsbuildinfo`. **P2.e closed the same session** — see §3, including the wrong row count it found in the work trail. **Nobody has opened a browser on any of the last six cards**, which is the standing shortfall and is bigger than any card in the file: P2.a, P2.b, P2.c, P2.l, P2.d and P2.e are all held by test processes alone. |
