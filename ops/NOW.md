...
switch; six cards are open. Eval spend by the open cards: $0.75** — each
...
### Wave 4 — the pages you keep

**The lead's prompt:** *"Read ops/NOW.md. Run wave 4 as a workflow: one agent per card, each in its
own worktree, then merge in order, measure, and push when every suite is green."*

- [x] **W4.1 a kept page reads like what it is** — **Eval: subset.** the owner, 2026-09-23, of the PAGES list (Estate Dashboard, Store Dashboard, Estate Week, AJI BARN Reorder): *"next thing we need to do is make how each page style work idealy"*. Today every kept page draws the same: a header and one ruled section per pin, each mounting its OWN board, so the answer page's document layout (dateline, heads, figure sizes, rows) never reaches a kept page (`KeptPage.tsx:312` passes no arrangement; `pageOf.ts:45` bails because pin blocks carry no claim) and `.r-kept` has no CSS rule at all. **The decision:** a page carries a KIND, declared in `pages.kinds` in the yaml, and code draws the page to it — `dashboard` (single-figure analyses group into one row of stat tiles, tables full width), `week` (one column, a dateline, a head per analysis), `list` (the rows are the point: tables full width, charts beside them small), `collection` (today's page, the default). **A kind decides how the page is DRAWN and never what is on it, what a figure says, or the order the person put the analyses in.** Stored on `george.pages.kind` with `kind_set_by`, set by Bob on `create_page` / `edit_page set_kind`, derived by code from the pins' own tools and shapes when unset, changed by the owner beside Rename. **Region:** a new `pages.kinds` block in the yaml, migration `c4d5e6f7a8b9`, the page services/routes and write tools; frontend `KeptPage.tsx` (the pin runs lift to the page so it can group them), the existing document renderer, `room.css`. **Done when:** each of the four kinds draws differently from the same pins, the owner can change a page's kind, the person's order never moves, and the four real pages are rendered at desktop and phone width. **Complete 2026-09-23, live at `6b18b2d`** (migration `c4d5e6f7a8b9`). The four real pages derive `dashboard`, `dashboard`, `week`, `list` off their own stored calls, read from the live rows. Frames at 1440 and 390 for all four kinds: a dashboard's run of single figures in one row with each read time kept, a week with a dateline and serif heads, a list at full width, a collection byte-for-byte as before; no accent anywhere, no sideways scroll. The lead added two things the halves left: the kinds speak the yaml's words (`kind_options`), and an analysis's six controls are one quiet disclosure, not a hover. **Not met:** how a kind DRAWS is in `room.css` and `room/pageKind.ts`, not the yaml — rule 3 is half honoured for this card. Nothing shows the kind on `/pages`.

- [ ] **W4.2 his own levels, and the alert that fires on them** — **Eval: subset.** the owner, 2026-09-23, on the third asking: *"i keep telling it to set levels for me and alert me when it reaches but its not doing anything"*. Bob is RIGHT that he cannot write a level into StoreHub (`product_stock_levels` is filled only by the export import; the API is used for barcodes alone) — and being right is not an answer. **The decision:** a level Bob works out is HIS, kept in `george`, never written to StoreHub. A yaml definition computes a proposed level per line and shop from what the line sells (`stock_cover` already holds speed, `units_per_day * window_days`, and `replenishment.review_period_days`) — code computes every digit, no model-invented threshold (rule 9, and rule 6's bounded setting: meaning, type, bounds, default, where it participates). Bob PROPOSES them (level four: he recommends, the owner decides) as a list the owner accepts, changes line by line, or rejects; accepted levels are stored with who set them and when, versioned. The existing `stock_running_out` watch and the morning line then fire against his levels for the 3,435 lines that have none, saying which level fired and that it is his, not StoreHub's. A StoreHub level, where one exists, always wins and is never overwritten. **Region:** a new `levels:` key in the yaml, a migration, a level writer injected by the web process (rule 4 — Bob holds no credential), `tools/stock_cover.py`, the watch condition, the brief line, and the frontend list where the owner accepts or changes them. **Done when:** "set the levels for me" returns proposed levels the owner can accept in one gesture, and "alert me when it reaches it" then creates a watch that backtests against those levels and is born off.

- [ ] **W4.3 a system you can run, backtest, promote and switch on** — **Eval: subset.** the owner, 2026-09-23: *"how do i turn it on and what its telling me to do?"* — asked of a screen that told him an administrator must promote it, when HE is the only administrator. Two defects under it (DOGFOOD, same date): Bob names the ROLE and never checks whether the person he is talking to holds it; and `/workflows` cannot promote — the button is in Needs you and only for a version already backtested, so a never-backtested version has no path anywhere. What else the page cannot do, against endpoints that all exist: see version history (`GET /{id}/versions`), see more than ONE run or open a run (`GET /{id}/runs/{run_id}`), backtest (`POST /{id}/run` with `as_of`), or switch a schedule on, off or repoint it (`PATCH /{id}/schedules/{sid}`). `diverges` is computed by `workflowShape` and dropped, so a version divergence (rule 8) has no weight on the page. Every rail row links to the list, not to the system. **Region:** a per-system route and page, `services/workflowsApi.ts` (the missing clients), `Rail.tsx` deep links, `workflowShape.ts`; Bob's side: name the PERSON who may promote and say when it is the person asking. **Rule 7 is the point and does not move:** promotion is an administrator's act against a RECORDED backtest of a closed window, a schedule pins a version id, and an edit makes a new ungated version. **Done when:** from one system's own page the owner can read its versions and runs, backtest a version, promote it if he may, and switch its schedule on — and Bob, asked "how do I turn it on", names him.

- [ ] **W4.4 the watches page** — **Eval: subset.** SYSTEMS and AUTOMATIONS · WATCHES both link to `/workflows`, which holds workflows: there is no page for a standing question or a watch anywhere in the app (`listStanding` is called in exactly one place, `Rail.tsx:30`). So `last_asked` and `last_status` are read and never drawn, `instructions[]` is invisible, there is no latest answer, no reschedule, rewrite or remove, and **no switch** outside the morning question's own button — the one thing the owner asked for. An off watch also loses its slot in the rail, so "off" and "Mon at 08:00" are two different shapes in one group. **Region:** a new route and page for the standing questions and watches, `services/standingApi.ts`, `Rail.tsx` deep links. Bob's writer already supports create / reschedule / add_instruction / rewrite / switch / remove — this is the surface for what he can already do. **Rule 7:** a schedule is born off and a person switches it on; nothing here promotes itself. **Done when:** every standing question and watch is on one page with its slot, its last run and what it last said, switched on or off in one gesture, and what it was told is readable.

- [ ] **W4.5 people, and the last legacy screen** — **Eval: none.** PEOPLE in the rail opens `/settings`, which is store filters and a cache button — nothing on it is about a person. The surface that holds accounts, roles and page access (`/admin/page-access`) is linked from nowhere, and W2.2's people (Joy, Isaiah, Daniel, Elijah) live in a third place, the Needs you page. The rail row is drawn unguarded, so a role without `settings` gets a link that bounces — the exact failure the Sources group takes care to avoid. `/settings` is also the last screen outside the room's vocabulary: raw Tailwind and hard-coded hex, no `RoomHead`, no `r-*`, errors falling back to invented strings, hard-coded cache-time prose with no timestamp (rule 6), and `stores.length === 0` rendering a permanent "Loading stores…" — loading and loaded-empty as one rendering, which rule 8 forbids. **Done when:** PEOPLE opens the people — the four of W2.2 and the logins they are linked to, with roles — in the room's own vocabulary, and no rail row can bounce.
