...
| Head | **`9261c2e` — pushed and live 2026-09-14.** Three commits: P1.i (`ed0a5eb`), this file recording it (`9261c2e`), and `fda8bad`, which had been committed and never pushed. **No migration.** Before them, `ab01579`. A docs commit recording the deploy sits above this row and is docs-only. |
| Live | **`9261c2e3`**, confirmed from `/health`: healthy, schema `w7x8y9z0a1b2` current and expected, deployment `93b527aa`, `schema_checked: cached`, **no migration**. **THE SWAP WAS WATCHED AND COST NOTHING — the first of six with no 502 in it.** The poll was started before the push again: `curl` every ~4.5 s from 12:17:50 UTC, the push at 12:18:37, 200 on `ab015794` through 12:19:40, and **200 on `9261c2e3` at 12:19:44** — **25 probes, 25 two-hundreds, no gap**. 67 s from push to swap, against ~50 s on `ab01579`. **The run of four consecutive one-502 swaps is broken, and the honest bound is the poll**: at 4.5 s resolution a blip shorter than that would not be seen, so this says no outage was OBSERVED, not that none occurred. Four of the five swaps before it carried a 502 that WAS one poll wide, so the resolution has been enough before. The route answers **401 against a 404 control**, so it exists on the live build; `/openapi.json` is 404 in production and the new request shape was not proved live, because that needs the owner's token. The 50-minute outage on `8b0325a` remains the migration path and is still unexplained. Read `/health` rather than believing this row. |
| Last deploy | `9261c2e`, and before it `ab01579`, `69d1fbf`, `1edf8fb`, `079359f` and `8ba0080`. **`9261c2e` is the first swap in six with no 502 observed**, and the second where the poll was running BEFORE the push, so it is observed rather than inferred. The four before it each cost exactly one 502 at the moment of the swap with 200s either side; `91dec69` ended the earlier run of five clean ones. None of the six carried a migration, so none of them is the path the 50-minute outage on `8b0325a` lives in. |
