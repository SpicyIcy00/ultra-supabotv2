...
| Phase | **1 is CLOSED — P1.a through P1.k and P1.✓, every card of it.** **Phase 2 is under way: P2.0, P2.a, P2.b, P2.c, P2.l, P2.d, P2.e, P2.f and P2.g are closed, four cards and the close remain.** **Its gate is further away again — the log's Open went from empty to five on 2026-09-15 and the count restarts from the first day it is empty. It was already further away than it had been that morning**: the log's Open must be empty **five days running**, it reached day two, and **2026-09-15 put two items back in it** — the colour question, and the claim-over-another-shop's-figure found underneath it. **The count is back to zero and starts again the first day Open is empty.** By the log's own rule nothing in section 3 is started while Open has anything in it, so the next session takes the log, not P2.d. The other three gate conditions are met or named — median 17.1 s against < 10 s with its cause on the card, a navigation fragment redraws with no model call at 760 ms, trust rows unchanged. **The Fable 5.1 review of Phase 1 HAS happened** (2026-09-14, recorded in `ops/DECISIONS.md`, confirmed by the owner 2026-09-15). It confirms every number in the close-out and names four things Phase 2 should not trust: **760 ms is a unit timer**, in-process with no HTTP, browser or render, so the gate's fragment condition is met by a measurement nobody has seen on the live build; **19.0 → 17.1 includes `correction` reading nothing at low effort**, and low is most turns in a one-thread UI; **effort is unobservable in production** — not persisted, and a revoked beta pins the process to high; and **five empty days are vacuous if the room is not used**. |
| Next card | **P2.m — "analyze" is an investigation, not a lookup.** Added 2026-09-15 off his own report and it is the top of the list, ahead of P2.h: he asked *"analyze tradsnax per store"*, got one chart, and said *"i feels very limited not limitless"*. **The cause is one sentence of the prompt** — INVESTIGATING opens *"'Why' is an investigation"*, so the ladder is gated on the WORD why — and the answer was inside its allowance, not against it: 4 reads permitted, 1 made; 2–4 findings asked for, 1 given. **It is the first Phase 2 card to need a live run** (eval: full, $1.51), because it changes the prompt and depth is the one thing the four gate scenarios cannot see. **THE DOGFOOD LOG STILL OUTRANKS THIS ROW AND HAS SIX OPEN.** Its own rule: nothing in §3 is started while Open has anything in it. **P2.g was done on 2026-09-15 against that rule, at the owner's explicit instruction** (*"Read ops/NOW.md. Do card P2.g."*) — recorded here rather than quietly, because the next session should not read the closed card as evidence the rule was relaxed. Two of the five are waiting on HIM, not on a session: the removed compare shortcut and the two-shop emphasis are both marked NOT VERIFIED BY ANYBODY. The top item a session can act on is **the tile explaining itself to the reader**. After the log, **P2.h voice**. And the finding that still outranks the card list:  Read from production 2026-09-15, read-only: **0 standing questions, 0 watches, 0 workflow schedules — none enabled, none ever run, none ever fired** — against 359 posts and 6 beliefs held. **So feature 12, Proactive intelligence, is marked `built` on the Ideal UI's own map and has never once spoken to the owner.** The machinery is live and correct: `scheduler.py` starts `standing_tick` and `watch_tick` on every boot and they have been ticking over an empty table for weeks. **Nothing is broken and nothing needs building — nobody has ever created one.** The only door is asking George in conversation (`set_standing_question`, the watch tools); there is no surface that offers it, and `view_automations` answers "nothing" to a person who has no way to know there was something to switch on. **The owner, asked which of four Jarvis axes matters, said 2026-09-15: "Initiative is needed"** — and initiative is the one already built and empty. That makes the next work a SWITCHING-ON card, not a build. **He also settled two open questions:** *"me and my family run these buisnesses"*, so multi-user is real and **S.6 people and permissions stops being optional**; and *"calenders, mail in the future but our main goal is to make how it works with the buisnesses top"*, so the standard stays a business operating system and the Jarvis direction is recorded, not substituted. Top log item is still **"compare" showing raw ids**. |
...
its own numbers. **Three live runs across 13 open cards, $4.53** — P2.m joined them 2026-09-15, off his report that an answer felt limited — the two
...
- [ ] **P2.m "analyze" is an investigation, not a lookup** — **his words,
      2026-09-15:** *"it only showed me 1 chart when i thought i would go in
      depth products per store and what i thinks … i feels very limited not
      limitless"*.
      **THE CAUSE IS ONE SENTENCE OF THE PROMPT**, and it is not a cap: the
      INVESTIGATING section opens *"'Why' is an investigation"*, so the ladder
      — verify, decompose, localize, explain, stop — is gated on **the word
      why**. "analyze tradsnax per store" names a subject and a dimension, so
      `investigation.scope` calls it **FOCUSED**, whose instruction is *"the
      smallest set that completely answers it"*. **George read once and said
      one thing against an allowance of 4 reads and 2–4 findings.** He was at
      the floor of his own policy, obeying it.
      **NOT A CATALOGUE PROBLEM, CHECKED:** six marks exist, a board holds 12
      objects, and LOCALIZE already reads `product_revenue` by product with
      `rank_by='biggest_drop'`. Every piece he expected is built. Nothing
      reached for them.
      Build: the ladder is climbed for an INTENT rather than for a word.
      `investigation.message_kinds` already has seven kinds — `question`
      ("answer it") and `intent` ("investigate what it implies") among them —
      and the INVESTIGATING section is connected to none of them. Connect it,
      and let "analyze", "look into", "break it down", "go deep" be what they
      plainly are. A FOCUSED message that asks to be decomposed gets a
      prescribed second read the way BROAD already gets a localization,
      instead of "the smallest set".
      Done when: "analyze tradsnax per store" returns the per-store read AND
      the decomposition under it, with what he thinks where he holds a view;
      "how did Rockwell do" still returns one read, because that one is a
      lookup and widening it would be the opposite mistake.
      **eval: full, once.** This changes the PROMPT and therefore how every
      answer is reached, and depth is the one thing the four gate scenarios
      cannot see — only the full eleven carry a question that climbs the
      ladder. The trust rows are what is at risk: more reads is more chances
      to state a figure no tool returned.
