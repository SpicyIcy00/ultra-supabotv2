...
- [ ] **P2.l colour means one thing** — **TAKEN BEFORE P2.d, which is why it
      is out of alphabetical order**: it came out of the dogfood log on
      2026-09-15 and the log outranks this list. **His words:** *"what do the
      colors mean now? does this make sense?"*, asked of `51af583` on the live
      build. It does not, and the answer is that colour means FOUR things at
      once — the tile's wash is IDENTITY (`identity.ts`, seven shops named by
      hand), the wash's brightness is MAGNITUDE with no direction (`--i`,
      |change| capped at 30%), and the dots, bars and pills are DIRECTION
      (`--up`/`--down`/`--flat`). **And three of the seven shop hues sit on
      top of the three semantic colours**: OPUS is amber (`222,138,11`) and so
      are George's own mark and the reserved approvals accent (`#D2691E`);
      Magnolia is rose (`224,68,102`) and `--down` on dark is pink
      (`255,92,138`); Greenhills is green and `--up` is green.
      `identity.ts` names the Greenhills collision in its own docstring and
      says the delta pill handles it — the pill is twelve pixels and the wash
      is the whole tile.
      **THIS IS P1.e'S REPORT ONE LAYER OUT.** That card took identity colour
      out of the MARKS — "colour is direction, not identity" — and deliberately
      left it on the tile SHELL, where `palette.test.ts` still permits it:
      *"identity keeps its hue where identity is the point — the tile's edge
      and wash"*. On the dark theme the shell is `--bloom: 0.62`, so the thing
      that was left alone is the loudest thing on the screen. The same
      complaint, at the layer the same card exempted.
      **The fix is the move P1.e already made, applied out one layer**: the
      shell stops carrying identity, and what a tile is about is read from its
      title — which every block already has — rather than from its colour. A
      hue stays where identity IS the point and nothing else is competing: the
      object panel, and a subject token. **Whether magnitude keeps a channel
      at all is the question to settle with evidence, not taste** — a wash
      that says "this moved a lot" without saying which way is the one channel
      here that nobody asked for.
      Done when: a board of seven shops draws no hue that decodes a name the
      row label already gives; `--up`/`--down`/`--flat` and the accent are the
      only colours on it that mean anything, each meaning one thing;
      `palette.test.ts` and `accentUse.test.ts` are extended to the shell
      rather than exempting it; the eight recorded runs replay through the
      real renderer with no fifth meaning. **No eval** — no model-facing file
      is touched.
