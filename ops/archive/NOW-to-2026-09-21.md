...
its own numbers. **Two live runs across 15 open cards, $3.02 — the two phase closes, and nothing else**: since 2026-09-18 no card runs its own test (the owner: *"revise the plan no more tests ill do it at the end of the phase"*), so the P2S.6, P2S.7, P2S.9, P2S.10 and P2S.11 subsets ($3.20 of the old $6.22) fold into P2S.✓'s full run (P2S.3's subset was spent 2026-09-17, $0.72 against $0.64) — P2.m closed 2026-09-16 and **its own run has not been made**, so the paragraph below it is the record of what that costs — the two
...
- [x] **P2S.2 the drawing** — three parts, one pass over `marks.tsx` /
      ...
      **Done when:** a dom test with a mocked canvas sees four distinct
      ...
      No eval.
      **CLOSED 2026-09-17, unpushed — `882ce09` (d), `2b44ee5` (e), `1e1eecd` (f),
      `bad1582` frames and the log's tile sentence, and this card's docs commit.**
      **(d)** `alive.ts` is the design's `draw()` split in two: `markStateOf` reads
      the state off the stream (a read running → reading; busy with nothing
      running → writing; figures still arriving → reading; a LOADED approvals
      count → need; else idle) and `drawMark` draws one frame. A pulse is a read
      landing or a figure arriving; a failed turn dashes a broken ring in his own
      ink. `?form=wide|round|tide`, wide the default. `AliveMark.tsx` joined the
      accent allowlist as the mark's exemption (4 of 4). **(e)** `identity.ts`
      names no store: a store's slot is its place in `stores.active_retail` as
      `/definitions/desk` serves `locations`, through `IdentityContext`; nothing
      before those load; past the eighth store no swatch; products slots 5–8 by
      FNV hash; categories, suppliers, warehouse none. The swatch is the ONLY
      element wearing a categorical hue — dumbbell, ranked and contributors rows,
      a table's subject cell, a line's series key, a remembered view; an opened
      object's rule takes the same hue. Palette = the dataviz skill's eight,
      validated on `#0f1011` (all pass) and `#f7f8f8` (pass, sub-3:1 WARN on slots
      3–5, relieved by the label that is always beside a swatch):
      `ops/palette/REPORT.md`, validator copied in, `palette.test.ts` holds
      room.css to those hexes. **(f)** rows rise 90 ms apart, bars and segments
      grow, dots fill, a line draws along `pathLength=1`, once on arrival;
      `data-v` on every mark kind (figure, dumbbell track, ranked bar,
      contributors bar, every line point, every numeric cell) with `data-read`
      off `snapshot_timestamp`; hover shows, tap pins and does not open the
      figure; the receipt line is a button that opens source and filters in
      place. **Frames** (`verification/frames/p2s2/`, 40 images + 10 touch
      shots): draw ← "taught", memory ← `ops/frames_fixtures/memory.json`, and
      situation/doing/nothing re-shot; all 20 frames centre offset **0 px**,
      580/940 and 433/703, no scrollbar; doing draws **21 swatches** and 23
      ...
      **SHORTFALLS:** no PIE exists until P2S.3, so "a dumbbell, a line, a bar
      and a pie" is held for three kinds plus a table; no current mark draws
      more than one series, so "a series carries its hue" has nothing to act on
      (a line's KEY carries it); the frames show the mark only IDLE — the four
      states are held by the dom test on a recording canvas, not by pixels; the
      memory frame is a fixture, because no eval has ever recorded
      `view_memory`, and it is the old memory tile with swatches and arrival, NOT
      the design's memory scene (timeline + holds); the tooltip is hover and tap,
      not keyboard; products share slots 5–7 with stores 5–7 by construction;
      identity greens (slots 3, 6) and red (8) sit near `--up`/`--down` on a
...
- [x] **P2S.3 the vocabulary, and "ruled out"** — alone, because it is the one
      ...
      **Done when:** each shape has a golden render test off recorded rows;
      ...
      gate scenarios pass.
      **CLOSED 2026-09-17, unpushed — `eeb7c7d` the vocabulary, `3af501e` (g), `dbf9dac`
      frames, and this card's docs commit.** **The vocabulary:** 17 marks — the six plus
      bar, multiples, area, stacked, pie, scatter, heatmap, calendar, waterfall, treemap,
      gauge. Each in `composition.widgets` with `when` (the rule that picks it unasked,
      taught on the compose schema), `rows` (what its read must carry,
      `composition.shape_rows`) and `min_rows`; pie, treemap, gauge `only_when_asked`
      (`asked_by`). **Map and funnel are DECLINED** (`composition.declined`): no store
      record has a coordinate, and no read returns the stages of one process — so **14 of
      the design's 16 shapes**. One rule on two sides, `agent/vocabulary.py` and
      `catalogue.drawable`, held to one matrix over `__fixtures__/vocab-reads.json` — one
      real read per shape, recorded by `ops/record_vocab_reads.py` with the vetted tools
      and no model. A shape the rows cannot make, or an unasked pie, is drawn as the rows'
      default and named in `meta.coerced`. "Make that one a pie" is a `change` naming a
      kind and no seq; asked once, it stays. A read grouped by two things now defaults to
      heatmap / multiples / stacked — it was a `line` joining every store's weeks.
      `ruled_out: true` on a block draws `READ n · RULED OUT`, dimmed. `markFor`'s seven
      retired names are KEPT, held by catalogue.test.ts "still draws every retired kind",
      because no migration rewrites stored boards.
      **(g):** a pin run returns `blocks` (`default_composition.pin_blocks`, the board's
      rule through the same gate); `room/KeptPage.tsx` draws them with `Board` and keeps
      every page control as a room control; `drawn_as` rides on a pin's stored calls, so
      ...
      **0 px**, 580/940 and 433/703, **0 clipped**, no scrollbar, no page scroll. Every page
      looked at; what they showed was fixed (bar names broke mid-word, a gauge overlapped
      its neighbour at 1440, a scatter drew unnamed dots and a smudge).
      **Gate: 4/4 passed, $0.72 against $0.64** (`verification/p2s3-gate.json`). First
      composed object 20.1 / 29.1 / 9.6 / 23.0 s against 15.3 / 38.6 / 14.9 / 37.8 s in
      `thoughts-gate.json` — one sample each, not a claim.
      **Suites:** pure 1,836 → **1,867**; vitest 71 / 1,017 → **65 / 916** (8 files of deleted
      modules gone, 2 added); `tsc -b --force` and `vite build` clean. The compose tool's
      ...
      **SHORTFALLS:** map and funnel declined, so 14 of 16; **on the four gate questions
      ...
      the "recorded run" behind the same-drawing test is recorded READS, not an eval turn;
      the frames script could not scroll the design's scene, so only its first screen
...
- [x] **P2S.6 Bob finds the why on a broad question — TAKEN BEFORE P2S.4** —
      ...
      *"NOTICE → INVESTIGATE → CONNECT THE EVIDENCE → FORM A VIEW → SHOW ME THE
      ...
      sales dashboard"*; *"present conclusions, not homework"*.
      **What is wrong, checked (the dogfood log, 2026-09-17):** `SYSTEM_PROMPT`'s HOW
      WIDE TO READ sends a BROAD question to *"net_sales, transaction_count,
      average_transaction_value grouped by store over a closed window, compared, at
      most 5 reads"*, and the ladder's LOCALIZE rung runs only for *"TAKING A FIGURE
      APART … 'why', 'what happened', 'analyze'"*. So "how are we doing" read three
      store reads, and "make me a store dashboard" four, and each stopped at *which*
      shop moved — then offered the localizing read as `next` and as a question to
      tap. It is also why P2S.3's shapes went unused: the reads they draw (by hour,
      by day, by category, by product) are the ones the rule never makes.
      **(a) a broad question climbs the ladder for what it finds.** Verify and
      decompose by store as today; then, for the shop or two that moved most by the
      tool's own ranking (`rank_by`, never a threshold he invents — if a cut-off is
      needed it is a definition in metrics.yaml), LOCALIZE in the same turn: the
      driver that moved, cut by time (day or hour) and by what sold (category or
      product), within a read budget for a broad turn declared in metrics.yaml. One
      finding decides the next read (§5); a shop that is fine is not dug into.
      **(b) a view, not a readback.** The answer says where the movement sits and
      what that means — what the reads establish, and what they do not (CLAUDE.md
      rule 10: localization is not cause; no attribution shares). `next` is kept for
      what no read could settle (a person, a document, a source), never for a read he
      could have made — held by an eval check on the gate turns.
      **(c) drawn in the shape the rule picks** — hours as a heatmap, days as a
      calendar or multiples, categories or products as stacked or contributors: the
      localizing reads are exactly P2S.3's `when` rules.
      **(d) the prompt pays for itself.** `SYSTEM_PROMPT` is 1,797 of 1,800 words:
      the change moves mechanics onto the tools' own descriptions (as P2.m did) and
      the budget test holds it.
      **(e) the cost is said.** More reads per broad turn is more seconds and more
      tokens: the median broad-turn time and the run's cost are reported against
      `p2s3-gate.json`, and the shortfall named, not hidden.
      **Done when:** on the live gate, "how are we doing" reads by store, then
      localizes the top mover by time and by what sold in the same turn, and its
      answer says where the drop sits; its `next` names nothing a read could have
      answered; at least one localizing read is drawn as one of P2S.3's shapes;
      P2.m's two unrun questions (*"analyze tradsnax per store"*, *"how did Rockwell
      do"*) are asked and read; every trust row unchanged (caveats surfaced, none
      forced, no figure no tool returned); the prompt at or under 1,800 words.
      **no eval of its own** — the four gate questions, plus the two above, costed.
      **WIDENED 2026-09-18 AT THE OWNER'S WORD, AND BUILT — UNPUSHED, THE LIVE
      CHECK NOT RUN.** Asked first to diagnose rather than build, the session
      found the defect was not one BROAD clause but the prompt's shape, and
      the owner said *"can we just optimze or system prompt i needs more
      initiative"*. Five places told him to stop, all changed: the persona
      (*"you say so little … and wait"* → he does the looking himself); the
      ladder's STOP on a dominant driver (now where LOCALIZE starts, both
      dimensions in ONE round); `next` (*"the one thing to check next"* → only
      what no read can settle, on the prompt AND on `compose`, whose `asks`
      no longer offer what the answer settled); findings (*"each off the same
      grouped read"* → a read made under the verified fact); and a LOOKUP
      (*"nothing under it"* → unless it moved — STANDARD §1's Rockwell).
      **Found and fixed: `broad.reads` never reached the model** — the
      prompt typed its own shorter sentence and dropped "then ONE
      localization"; it renders the yaml now, a sentinel test holds it. A
      broad question reads `get_attention` in its first round (not sales
      alone, STANDARD §2); `broad.max_reads` 5 → 7. **Cost, at his word**
      (*"i care more about cost then fast asnwers"*): every round re-caches
      the conversation (62% of spend, 7-day report 2026-09-18), so localizing
      is one round, and a broad turn is held to
      `broad.eval_cost_ceiling_usd` **$0.50** (measured $0.33–0.48 on
      2026-09-12..17). **Gate: a fifth question, "how are we doing?"**, with
      two checks read off the turn at no cost — a read below the shop
      (`checks.localizing_reads`) and the ceiling (`harness.turn_usd`); the
      thread's "how are we doing?" carries the same. **Prompt 1,797 → 1,787
      words, prohibitions 13 → 12** — paid for by moving mechanics to
      `get_sales` and `view_page` and cutting repeats; every numbered rule
      byte-identical in meaning, rule 9 restored whole when its test caught
      the cut. **Suites: pure 1,867 → 1,875** (+8), 4 tests rewritten to the
      new rules (stop reasons, lookup, broad reads, findings).
      **SHORTFALLS:** the live gate is not run (≈ $1.20 for five turns) — so
      NOBODY KNOWS YET whether he localizes, what a broad turn now costs, or
      ...
      **CLOSED 2026-09-18 AS BUILT, AND LIVE** (the owner, 2026-09-18: *"revise the plan no more tests ill do it at the end of the phase"*). Everything it
      built is on `main` and deployed; the one condition left was its own
      trust run, and that now happens once, in the P2S.✓ full run — which
...
- [x] **P2S.7 Bob builds the answer in front of you — and the fixes P2S.6's
      ...
      **Done when:** on the live gate, "how are we doing?" shows the shops'
      ...
      saved. **no eval of its own** — the five gate and two depth turns.
      **BUILT 2026-09-18, UNPUSHED — NOT CLOSED: run 2 failed one trust row,
      and both runs are spent.** (a) A compose FOLDS into the turn's board
      (`compose.fold`) instead of replacing it — found underneath: every
      compose frame used to replace the turn's composition, so a second
      compose that only changed one key erased the first (p2s6 `why`). Reads
      that land later are drawn quiet at the end (`compose_added`, the
      once-a-turn latch gone); the client stopped reversing a turn's own order
      (`board.ts` unshift); the lead is settled by his first compose; the
      prompt says "compose as you go". **Found live and fixed: the loop's
      answer was the LAST round's text only** — prose beside a compose stayed
      on screen but the gates and the stored post never saw it ("overnight.My
      read", and P2S.6's "numbers.I'd"). (b) rounded slot figure said exactly
      when one read figure rounds to it; share-shaped sentences caught
      (`checks.share_of_a_change`, replayed: exactly the 3 recorded cases, 0
      false on p2s3/p1close); deletions never strand a sentence
      (`prose.strands`); the cap's forced answer names no tool; categories
      resolved ignoring case, refused with all 17 named. (d) claim/thought/
      reason cut, caveat/next/asks kept whole, emphasis capped — `over_length`
      in the yaml. (e) `filters_applied` is a LIST in production and
      `_scope_values` read only a mapping — fixed; foot-traffic explanations
      not flagged; a cut list says absence is not known; a grouped stock count
      states its whole; the report prints on cp1252.
      **Live, two runs:** `p2s7-gate.json` $2.56, 2/7; `p2s7-gate-2.json`
      $1.91, **6/7** (was 5/7 and 5/7 on P2S.6). Refusals not about truth
      5, 4 → **0, 0**. Broad: shops at 10.6 s, deeper blocks at **25.0 and
      36.1 s** (was nothing until 59.6 s), done 85.3 s (was 105.7), 244 words
      (was 503). Rounds 37 → 32, corrective 1 → 0. **SHORTFALLS:** run 2's
      `morning` said "₱25–45k" (a range no read returned) and moved its board
      (the lead restated as supporting — fixed after, with a test, NOT seen
      live); orphans in run 2's `morning` and `analyze` — a deleted paragraph
      opener, protected after, NOT seen live. Run 1's `analyze` cost $0.85:
      the category now resolves, and `get_product(category=)` returned 365
...
- [x] **P2S.9 fewer rounds for the same work** — the owner, 2026-09-18: *"is
      ...
      **(a) no empty last round** — when a round's only calls are compose and
      ...
      cheaper), the shortfall named. **no eval — rides the P2S.✓ close** (the owner, 2026-09-18: *"revise the plan no more tests ill do it at the end of the phase"*); its numbers are measured there.
      **BUILT 2026-09-18, UNPUSHED — CLOSED AS BUILT; its numbers are the
      P2S.✓ run's.** (a) `agent/loop.py`: a round whose only calls are
      compose, each standing whole (no block, slot or action refused), one
      naming the claim, with words beside it, IS the answer — the next pass
      runs the gates with no request; a gate's rewrite still gets its round,
      results and request joined into one user turn (`_join_user_turns`).
      The compose tool now says so (`rounds.settle.tool_sentence`). `done`
      carries `rounds_saved`. (b) `get_sales(metric='sales_headline')` is ONE
      call for the model and three ordinary reads for everything else — own
      seq, frames, board object, pin (`_expand_sets`,
      `metric_sets.sales_headline.asked_as_one_call`); one tool_result back
      with the three side by side. get_sales itself still refuses a set name:
      no second calculation path, `executes_nothing` still true. The broad
      policy names the one call. (c) `agent/model_receipts.py`: the model's
      copy of each meta sends a repeated note once a turn, then "same as call
      N"; source, filters, window, read time, every notice, error and
      truncation whole every time; `definitions_path` dropped for him;
      frames, logs, posts untouched (`rounds.model_receipts`). The eval
      report's `chars_to_model` is now what he was sent, `chars_full` the old
      figure. **Offline estimate on `p2s7-gate-2.json`, against the card's:**
      (a) all 7 turns ended on a closing round after compose — 42.5 s of
      421.7 s (10.1%), IF every turn settles; (c) **result characters
      185,604 → 167,171, −9.9%** — meta −16%, not "most of 59%": filters,
      comparisons and notices, kept whole by the card's own rule, are most of
      the meta. So the 10–15% cheaper is at risk; (b) saves one call's
      output and two metas per set, not rounds (the three were already one
      batch). **Trust risks for P2S.✓ to look at:** an answer cut short when
      he writes a preamble beside a claim-naming compose; a pointer where he
      expected a comparison block. Pure 1,927 → 1,942 here. Prompt 1,798 →
      1,796 words, sha `c59c7ac5` → `bb014fe0`. No migration, no frontend.
- [x] **P2S.10 three tools where he now makes many reads** — the owner,
      ...
      **Done when:** the log count is in the card's close; on the live gate
      ...
      cheaper on those three). **no eval — rides the P2S.✓ close** (the owner, 2026-09-18: *"revise the plan no more tests ill do it at the end of the phase"*); its numbers are measured there.
      **BUILT 2026-09-18, UNPUSHED — CLOSED AS BUILT; its numbers are the
      P2S.✓ run's.** **The log, first** (`george.tool_calls` joined to
      `george.conversations`, read-only, 30 days): the owner (`admin`) asked
      86 turns, 78 of them read, 351 calls. Every shop's drivers compared: 16
      turns, 46 calls, as 2+ calls in 15 (all before P2S.9 made them one).
      A shop's drivers: 6 turns, 21 calls, 4 of them with the products that
      moved in the same turn. Stock reads: `get_stock` 11 turns,
      `get_dead_stock` 7, `get_stock_history` 4 (the shelf check after a
      product fell, ranked `most_days_out`), `get_replenishment` 4; two or
      more stock tools in 8. By day: 4. The `coverage` user's 145 turns are
      the sweep's, not his. **TWO TOOLS, NOT THREE:** *every shop at a glance*
      is `get_sales(metric='sales_headline', group_by='store')`, one call since
      P2S.9(b) — a second name would be a choice, not a saving.
      **(1) `get_change(store?, category?, date_range?, compare_to?)`** — seven
      reads in one call: the headline compared (three), the window's days with
      the baseline's own before them (one series, never differenced), the five
      products that fell and the five that rose, and the shop's stockouts over
      the window (`most_days_out`, 10). With a category, four: its revenue by
      shop, by day, and its products that fell and rose — net sales cannot be
      cut to a category and stock history has no category filter.
      `compare_to` is `previous_period` or `same_weekday_last_week` (the
      morning's). **(2) `get_stock_health(store?)`** — four: lines per state per
      shop, the emptiest ten (negative counts lead), the replenishment summary
      and what it most wants shipped. **How:** `metrics.yaml one_call_reads`
      lists each tool's reads; `agent/one_call.py` fills in window and scope
      (no SQL, no connection, no read consumes another's rows); the loop runs
      them as ordinary calls — own seq, frames, board object, receipts, pin —
      exactly as P2S.9's set, one tool_result back with each read's `part`.
      Not in `TOOL_FUNCTIONS`, so a pin holds the reads, never the call. A
      ...
      in one call." **Every read ran live, read-only**: a shop's `get_change` is
      7 reads, ~27k characters to the model, 3.6–10 s run one after another;
      the estate's 31k; a category's 18.6k; `get_stock_health` 14.1k, and at
      AJI BARN its two plan reads refuse (a warehouse has no plan) while the
      shelf reads stand. **Also in this session:** the dogfood log's top item
      (`fd61f5c`, a missing tool argument refused instead of crashing), and
      the owner's core questions wired under `-m core` (2 and 5 new, run only
      when asked so the full run stays $1.51; 3 is P2S.11's).
      **Offline estimate on `p2s7-gate-2.json`, against the card's "about 30 s
      and 15–25% cheaper on those three":** broad saves its CHECK round, 11.1
      of 85.3 s (13%), and almost no money — the saved round (~$0.03) is
      offset by a bigger result (+8.8k characters over the four reads it
      replaces); the morning saves about one round, 13–17 s of 83.6, ~9%;
      analyze, if he asks `get_change(category=…)`, 7 → 3–4 rounds, 40–60 s
      of 117 and ~30%, part of which P2S.7's category fix already bought.
      **So ~22–31 s a turn on average, and 15–25% cheaper is at risk on the
      broad turn.** **Trust risks for P2S.✓:** he reads the shelf rows as
      "the products that fell were on the shelf" (a product not in the top
      ten is not known either way — the read's `full_row_count` says so);
      a figure differenced across the day series. Pure 1,942 → **1,965**
      here (1,961 in CI); vitest 957; `tsc -b --force` clean. Prompt 1,796 →
      1,796 words, sha `bb014fe0` → `b1be6950`; the tool list gains two, so
      the cached prefix is rewritten once. No migration; frontend only
      `work.ts`'s words.
- [x] **P2S.11 what you tell him, he keeps and acts on** — the owner,
      ...
      **Done when:** "don't focus on per gram, it doesn't matter" in one
      ...
      live gate. **no eval — rides the P2S.✓ close** (the owner, 2026-09-18: *"revise the plan no more tests ill do it at the end of the phase"*); its numbers are measured there.
      **BUILT 2026-09-18, UNPUSHED — CLOSED AS BUILT; its numbers are the
      P2S.✓ run's.** **(a)** `belief_store.current` lists told views first
      and `as_block`/`in_prompt` carry every told view (own cap,
      `MAX_TOLD_IN_PROMPT` 24) and then his newest 12 — the 40-newer contract
      holds. The memory screen needed no change: told rows were already drawn
      with Forget, and now lead its twenty. **(b)** a SEVENTH stance,
      `leave_out` (taught, like `means`: `told`, no calls — the table's
      one-ground check already fits, **no migration**), binds the FIRST
      `settings.declared` entry, `left_out_categories` (all five contract
      fields, max 5). The category is checked against the catalogue when
      told and kept in its spelling; an unknown one is refused.
      `belief_store.bound_settings` turns standing views into the value;
      `/ask`, the standing runner and the object tap pass it; the loop hands
      it as keyword-only `left_out` to exactly the reads the declaration
      names (get_sales, get_stock, get_stock_history, get_replenishment,
      get_dead_stock, get_object) and DROPS one the model sends. **LISTS,
      NEVER TOTALS** (the session's call, in the yaml): a list of products
      or categories leaves it out and `filters_applied` says *"per gram
      left out at your instruction, <date>"*; a total (a shop, a day, the
      estate, a count per state) is untouched and `meta.settings` says why;
      a read that names the category or one product leaves nothing out and
      says so. get_change and get_stock_health apply it through their reads.
      "Include per gram again" supersedes the view with a `means` view;
      Forget unbinds too. **(c)** on `record_belief`'s description, not the
      prompt (the budget test's own rule). **Live, read-only, 2026-09-18**,
      rows with per gram before → after: category breakdown 12 → 11 rows;
      North Edsa out of stock 5 → 0 of 50 (full set 2,966 → 2,765);
      stockouts 1 → 0 of 20 (2,983 → 2,802); plan 2 → 0 (133 → 131); dead
      stock 9 → 0 of 50 (336 → 327); estate products-that-fell full set 523
      → 474; North Edsa sold no per gram last week, so "what fell at North
      Edsa" reads the same with a receipt. Not done: a PIN or workflow step
      re-runs its stored call without the setting (its receipt then says
      nothing was left out, which is true); get_movement, get_product and
      get_attention do not take it. Pure 1,965 → **1,990** (+25). Prompt
...
- [x] **P2S.4 same-store year-over-year** — **was P2.i, kept by the merge
      ...
      **BUILT AND CLOSED 2026-09-18, UNPUSHED. THE CARD'S OWN QUESTION CANNOT
      BE ANSWERED, AND THE TOOL NOW SAYS WHY: THE SALES RECORD HAS NO DECEMBER
      2024.** Measured building it: every shop sold every day of August 2024
      and January 2025, and between them the record holds 5 days of September
      2024, 1 of October, **none of November or December**. So "last December
      against the year before" refuses — *"The sales record holds no sale from
      any shop between 2024-12-01 and 2024-12-31 … missing from the record or
      every shop was shut"* — rather than blaming eight shops. Filling the
      hole is a source only the owner has (a StoreHub export of Sep–Dec 2024);
      logged in DOGFOOD_LOG. **What was built:** `same_store` in metrics.yaml
      (a shop counts when its first sale on record is on or before the
      earlier window's first day, its last on or after the later window's
      last day, and it sold in each; five named reasons otherwise;
      `confirmed_by_owner: false` — **the wording waits on him**) and
      `comparisons.same_period_last_year` (inherits `previous_period`; the
      same calendar dates a year back, 29 Feb handled; closed windows only —
      `same_elapsed_last_year` is recorded as not built). `get_sales` judges
      the shops in one statement in the read's transaction, narrows
      `store_ids` for BOTH windows, rewrites the scope line of
      `filters_applied` to name the counted shops and every one left out with
      its dates, and puts the same in `meta.comparison.same_store`. Closed
      shops are judged too when no shop is named, so AJI PINA is left out BY
      NAME. **Beyond the card, because the hole made it necessary:** a record
      check — days in each window on which no shop sold. None → refused;
      some → `sales_record_silent_days`, drawn (data may be wrong): without
      it September 2025 against a 5-day September 2024 read **+567%** for
      Rockwell. `same_store_scope` (explains only, not drawn) says the figure
      is same-store. **Done-when, against the numbers:** December — NOT MET,
      refused for the record, above; the comparable set named — MET on
      August 2026 vs 2025: Rockwell, Fairview, Greenhills, North Edsa,
      Magnolia counted, OPUS, Shangri-La, AJI PINA left out by name,
      −33.8% (4,581,914.01 vs 6,919,157.17 — equal to the plain read over
      those five in each year); a shop that opened mid-window excluded by
      name — MET on September: North Edsa (first sale on record 2024-09-02)
      and OPUS (2025-09-30) named in the receipt. Pure 1,990 → **2,014**
      (+24, `test_same_store_contract.py`); golden +8 (4 table rows, 4
      tests), all passing live; **5 golden/live tests fail on HEAD without
      this change** (four say "Shang", renamed Shangri-La 2026-09-10; one
      reads `selection.comparison`, absent) — logged, not fixed. Prompt
...
- [x] **P2S.5 voice — a first-class way of operating Bob** — **restored by
      ...
      **BUILT AND CLOSED 2026-09-18, UNPUSHED.** `room/voice.ts` holds the
      rules (which browser hears, what was heard so far, which failure is
      which, the claim sentence), `room/Mic.tsx` the browser half (the mic,
      the hands-free switch, the reader). **Speaking is not a second path:**
      the heard words go into the line as they come and are sent by
      `onSay`, which Room points at the same `ask` Enter reaches — so the
      selection, estate, page and board travel as typed, and a steer
      resolves through `resolveFragment`. Hold ≥350 ms and release sends;
      a tap listens until the next tap and leaves the words to correct
      (with hands-free on, it ends at the recogniser's settled phrase and
      sends). Hands-free reads `spokenClaim` — the headline sentence exactly
      as `Reading` cuts it, markers out — only on a busy→idle edge, so an
      opened thread is never read at you; the claim wears a ring
      (`data-speaking`), and any pointerdown or keydown stops him. "Read it
      to me" is `surface.desk.fragments.read_aloud` in metrics.yaml, served
      verbatim, asks nothing of the model. **Done-when, against the card:**
      hold / "how are we doing" / release — MET (dom test); two shops and a
      spoken "compare these" — MET on the REAL `Room` mounted in jsdom for
      the first time (the request carries both store ids, not the third);
      a spoken steer replays with no model call — MET for "last 30 days"
      (the replay POST, `ask` never called); **"last 90 days" — NOT MET,
      and cannot be by voice: there is no 90-day window in
      `sales_day.presets`, so it is a question typed or spoken** (a test
      says so); hands-free reads the claim only and a tap stops it — MET
      on mocked synthesis; no recogniser, no mic and one line — MET;
      the frame — MET, `ops/frames.py --scenes doing --voice`
      (`verification/frames/p2s5/doing-{1440,1920}-open-voice.png`: Rockwell
      and OPUS chipped, "compare these two" in the line, the mic ringed
      between the chips and ↑ where the design's ◉ is). **Found by the frame,
      not a test:** headless Chrome ships the unprefixed `SpeechRecognition`
      ...
      vitest **957 → 986** (+29: `voice.test.ts` 10, `voice.dom.test.tsx`
      19), pure **2,014 → 2,015** (+1, `read_aloud` served and colliding with
      no steer), `tsc -b --force` 0, `npm run build` ok. Prompt byte-identical,
      1,800 words sha `eec644cb`; no migration; the only backend file is
      metrics.yaml, nothing on the model path reads `fragments`.
- [x] **P2S.✓ close: it feels right** — **the one close for all of Phase 2,
      ...
      subset cannot see either. **And since 2026-09-18 it is the ONLY run left
      in the phase** — no card runs its own test, at the owner's word — so it
      checks everything P2S.6, P2S.7, P2S.9, P2S.10 and P2S.11 changed: every
      trust row on the seven questions, the rounds and seconds P2S.9 and P2S.10
      promise against `verification/p2s7-gate-2.json`, and P2S.11's per-gram
...
**Phase 3 — operating mode.** **RE-CUT 2026-09-19 at the owner's word** (*"go
and rewrite the plan"*), from nine sessions to six: four screens dropped or
merged — the UI is frozen since 2026-09-18 — every remaining card is Bob
doing something, and each is checked against the design's own scene rather
than a description. What exists and has never been switched on: the
standing-question runner, the watch runner and the workflow scheduler —
production holds **0 standing questions, 0 watches, 0 schedules, 1 workflow**
(read 2026-09-19). The owner, the same day: *"disclaimers can be hid"*,
*"default should be aji ichiban"*, the attention read's internal names, and
the cut-off hover — the first three fold into P3.b; the hover is a fix.
Dropped: P3.a (a queue screen, keys and a snooze migration for a queue that
holds nothing yet — its verbs live in P3.d), P3.c (page polish on a frozen
UI), P3.e (into P3.d), P3.f (into P3.b).

- [x] **P3.h StoreHub exports in — the API, or the upload page** — FIRST,
      because the morning of 2026-09-19 says purchase orders are 16 days old
      and stock transfers 80, and the ordering turn in the dogfood log needs
      them. **Half exists** (`51391e7`): `POST /storehub-imports/{kind}` for
      `purchase_orders` and `stock_transfers`, one transaction per file, a
      re-upload converges, an untrustworthy file is refused whole with the
      reason, every import a ledger row (`GET /storehub-imports`). **(a) the
      API first** — establish from StoreHub's own API documentation whether
      purchase orders, stock transfers and products can be read, never from
      memory; a kind it offers becomes a scheduled pull through the same
      `import_file` path and the card says so. **(b) the page** —
      `/storehub-imports` in the room's chrome (page key `storehub_imports`,
      granted): choose the kind, drop the file, the result drawn from the
      response (inserted, updated, deleted, unmatched and ambiguous SKUs,
      header-total mismatches, the notices), the ledger of earlier imports
      below from a loaded list (UI rule 8); the 11-column document-only
      transfer export refused by name. **(c) products** — find how
      `products` is filled today before adding a kind. **Done when:** a PO
      export uploaded from the page lands and the same file uploaded again
      inserts nothing; a document-only transfer export is refused with the
      reason; the ledger lists both; `get_purchasing` reads the new orders;
      the API question is answered with its source. No eval — nothing on the
      model path changes.
 **BUILT AND CLOSED AS BUILT 2026-09-19. THE PAGE IS PUSHED AND LIVE (`2201e28`); THE PRODUCTS KIND BUILT AFTER IT IS UNPUSHED AND CARRIES A MIGRATION — the live half of the done-when is the owner's and is NOT done.** **(a) THE API, answered with its source:** StoreHub publishes no API reference — searched 2026-09-19, the only first-party statement is its help centre's: an API key is requested from Customer Care by the account's master email (care.storehub.com, *Financio: How to Integrate with StoreHub*). The API itself, asked without credentials, separates a route it has from one it does not: `/products`, `/transactions`, `/customers`, `/stores`, `/employees`, `/timesheets`, `/inventory/{storeId}` and **`/purchaseOrders` answer 403 Forbidden**; `/stockTransfers` and every other spelling tried of transfers, stock takes, returns, suppliers and categories answer 404 `ResourceNotFound`. So: **purchase orders and products look readable, stock transfers do not, and none of it is documented.** No pull was built — `STOREHUB_USERNAME` / `STOREHUB_API_TOKEN` are unset here, the response shape is unknown, and `import_file` takes the CSV's bytes, so a pull is an adapter written against a payload nobody has seen. It is source **S.12** below. **(b) THE PAGE:** `/storehub-imports` in the room's chrome under its OWN key `storehub_imports` (`StorehubImportsPage.tsx`, `storehubImportsApi.ts`); in `PAGES`, and reached from BOB'S RAIL — a **Sources · last import** group naming the two records with the date each last arrived, absent and unread for a role without the key. It was in the Supabot sidebar until the owner read this close-out: *"it should be a page in bob not supabot"* — the route already rendered in his chrome, so the link was a door into his room from somewhere else. The links stay drawn when nothing has been imported (`never`), because that is the day the page is most needed; only the date is loaded, and `test_storehub_imports_route_contract` now holds the absence from the Supabot nav as well as the presence in the rail. Choose the kind, drop or choose the file, it goes up at once; the result is the response — documents and lines inserted, updated, deleted, unmatched and ambiguous SKUs, header-total mismatches, unresolved locations — with the server's notices ABOVE the counts, timed by its own ledger row; a 422 is drawn as *Refused — nothing was imported* in the parser's sentence and any other failure as an upload that did not go through; the ledger below is checking / could not be read / no file yet / rows, and the import drawn above is not repeated under *Earlier imports*. No accent anywhere. **The 11-column export is refused BY NAME:** `storehub.stock_transfers.refused_shapes.document_only` in metrics.yaml holds its exact header and the sentence; the parser raises that sentence and nothing else, before the generic mismatch. The real `Stock_Transfer_09-03-2026.csv` is refused with it and the real `Purchase_Orders_09-03-2026.csv` parses (58 documents). **(c) PRODUCTS: declined, then ASKED FOR AND BUILT.** `products` is filled by a job outside this repository — 321 rows created at 15:00–15:01 UTC over sixty days, 3,699 touched at 15:01 on 2026-09-18 — so the card's answer was that an upload kind would be a second writer on one table. Then the owner, having read it: *"we need a new one for products it needs to update supabases product page cause we have suppliers"*, with the real export attached. **The rule between the two writers is the whole design:** the nightly job owns name, SKU, category, barcode, cost, price and the flags; **this import writes NO `products` column at all** and takes only the two facts that job does not carry — the SUPPLIER LIST (`product_suppliers`, order kept, never tidied or deduplicated against a master that does not exist; SH1 "Aji Mix" has five) and the PER-STORE WARNING / IDEAL LEVELS (`product_stock_levels`, `store_id` NOT NULL, both nullable because blank means nobody ever set one and 0 means somebody set zero). Two tables, not two columns: a product has several suppliers and a level is per shop, and either as a delimited string is how "Seikyo SEK001; GZ Cri GZ001" becomes one supplier's name. **The column list cannot be fixed** — StoreHub writes three columns per STORE, named after it, between a 19-column leading block and an 8-column trailing one, so the real export is 84 wide and grows by three when a shop opens; the fixed blocks are matched exactly and the middle read as triples, and a renamed, dropped or reordered column is refused rather than read positionally. `#Required` under the header is StoreHub's own instruction row and is skipped by its first cell, before the field count, because its width is StoreHub's business. A product id with no catalogue row is COUNTED AND SAID, never created; so is a level at "Test stoee", which is in the export and in no alias. A re-import CONVERGES on the file, scoped to the products the file mentions. The ledger gained `counters` (JSONB) rather than reporting a product count in a column named `documents_seen`, and the page draws products in products — never "0 documents" about a file that has none. **NOT RUN AGAINST A DATABASE WHEN IT WAS BUILT:** the migration `y9z0a1b2c3d4` is the first new DDL since `x8y9z0a1b2c3` and there is no local Postgres, so it first ran on Railway at deploy; the importer is held by a recording session that compiles every statement it issues. **IT HAS NOW RUN FOR REAL.** The owner uploaded all three exports on 2026-09-19 at 20:05-20:06 Manila: orders 17 documents / 39 lines, transfers 71 / 1,253, products 3,699 matched with 2,381 supplier links and 40 stock levels, and a SECOND products upload converged exactly — 0 inserted, 2,381 and 40 updated, 0 deleted, which is the idempotency promise proven on live data. **AND BOB COULD NOT SEE ANY OF IT**, which the owner asked about directly: `george_ro` had no SELECT on the two new tables because `tools/george_ro_role.sql` is run BY HAND and no deploy runs it, and no tool read them either. Both closed the same day — the grant run against the live database, and `get_product` now attaching the supplier list (export order, names never normalised) and the per-store warning / ideal levels through two vetted query constants. **The two supplier answers are kept apart by name in every result:** `product_suppliers` is who StoreHub RECORDS, `definitions/product_suppliers.yaml` is who we have BOUGHT from and is what the purchase plan still uses; which wins for a purchasing decision is a definition **the owner has not made**. **`get_purchasing` reads `purchase_orders + purchase_order_lines` directly** (called read-only 2026-09-19), so a landed file is read with no further change. **LOOKED AT** in headless Chrome, the real page in the real shell over canned responses (`verification/frames/p3h/`): result, refusal and empty at 1440, and the result at a true 390 in an iframe (Chrome on Windows will not open a window under ~500) — no clipping, the long filename wraps. **Suites:** pure 2,031 → 2,039 at the push, → 2,063 with the products kind, → **2,064** after the byte-order-mark fix (local); vitest 70 files / 986 → 71 / 997 → 999 → **72 / 1,006**; `tsc -b --force` clean at each. **LIVE, AND BROKEN, AND FIXED:** the owner uploaded all three on `2201e28` and said *"its not working i think products is the only one working"* — StoreHub now writes its exports with a UTF-8 BYTE ORDER MARK, so the first header cell was `﻿P.O ID` and both document kinds were refused with a complaint naming a column that was there; products alone happened to carry no mark. Stripped once at decode for every kind, and his real orders export now parses to 17 documents / 39 lines with the mark and without it. Four things he asked for in the same message are built: **the line is on every screen** (the room shell's, in the board composer's own wrapper and place, naming the screen it was asked from), **the ledger is per tab** (read per kind from the server), **earlier imports are folded behind one line**, and **an upload draws a bar** — a real percentage while it is sending, a moving stripe while the server reads — **landing green and refusing red** on a third colour family, `--landed` / `--refused`, held apart from the accent and from `--up`/`--down` by `accentUse.test.ts`. **NOT DONE, and only he can:** no file has gone up through the live page — it is unpushed, the newest order held is 2026-09-03 and the newest transfer 2026-07-01, and a fresh export is his to make. Whether `storehub_imports` is switched on for his role could not be read (`george_ro` may not read `role_page_access`). Unverified: a transfer export over ~4.5 MB through Vercel's proxy (the largest real one is 3.6 MB). No local Postgres is installed, so the converge was not re-run against a database this session; it is `51391e7`'s, proven on the ledger's imports 3–8.
- [ ] **P3.b the morning, unasked** **AND IT IS THE FASTEST ANSWER THERE IS (2026-09-21):** the data syncs once a night and he asked the broad question seven times that day against identical data, redoing every read each time; answered at the slot you name and reused until the data changes (stamped with its read time, rule 6), your most common question costs nothing when you open the app. Pairs with P17.a. — the design's `morning` scene (§1, §10):
      Bob speaks first. **(a)** `usual_weekday` in metrics.yaml — the same
      weekday over the last N closed weeks: low, high and the middle band,
      computed in the tool — the definition the morning's "usual" rests on,
      before anything draws it (was P3.f; the band mark waits). **(b)** the
      FIRST standing question switched on: the morning, at the slot the owner
      names, born off and switched on by him (rule 7), its answer waiting in
      the room when he opens it — the cold open that is empty since
      2026-09-19 is where it lands. **(c)** the attention read drawn as
      words: no `sales_vs_same_weekday|Fairview|`, no MEASURE column, no
      "kept no" — a shop's row says *Fairview took ₱9,631 against its usual
      Thursday ₱22,981* from the read's own fields, a stock row is a line and
      a shop, both from `tools/attention.py`'s rows and never from the model.
      **(d)** notices folded: a notice that says a figure may be wrong is
      still drawn above it (UI rule 4), collapsed to its first line with a
      way to open, and "which ones" opens the list. **(e)** the estate switch
      opens on Aji Ichiban. **Done when:** the room opens on the morning's
      answer with no question typed; the attention figure carries no internal
      name; the notices fold and open; the switch opens on Aji Ichiban; a
      contract test holds the definition and the drawing. No eval — it rides
      P3.✓.
- [ ] **P3.g the stock watch fires before the stock-out, not after** — was
      P2.j, moved by the merge 2026-09-17: a watch is operating mode (§10,
      §13) and touches no surface. Today a watch fires when a line crosses
      zero, which reports a stock-out that has already cost the sale. The
      useful condition is "will cross zero before it can be restocked". The
      primitives exist: `tools/replenishment.py`, `tools/purchase_plan.py`
      and a units/week rate over closed weeks. The missing input is LEAD
      TIME, and it does NOT need the frozen PO export: rule 6 allows a
      definition to declare a **bounded setting** a person binds and every
      run records — "Seikyo takes 3 weeks" typed once, with bounds, in
      metrics.yaml. A line with no lead time set is reported as having none,
      never defaulted. **And the FIRST watch switched on**, on AJI BARN, by
      the owner. **Done when:** the watch fires for a line still above zero
      whose cover is under its supplier's lead time, naming both numbers; a
      line with no lead time says so instead of firing; the rail lists the
      watch with its cadence and when it last spoke. No eval — a scheduled
      watch makes no model call (rule 7).
- [ ] **P3.d build it and keep it** — the design's `build`, `life` and
      `decide` scenes (§11, §12, §15); one card where P3.d and P3.e were two.
      Consecutive turns sharing a subject render as ONE object with versions,
      revised in place, version arrows back; every proposed write in one
      PROVISIONAL frame — **Keep it · Change something · I want this every
      Monday** — that turns solid only on the write's confirmation frame; an
      edit to a rule, page or standing question drawn as before/after of its
      arguments over two versions, never prose alone (*v1 → v2: + lead_time
      (Judy, 21 days, you told me) · everything else unchanged*); Keep as
      version 2 makes an ungated version; Backtest first queues one; the
      schedule still pins v1 and the divergence notice says so; **Systems and
      Automations in the rail open the object, not `/workflows`** (P2S.1's
      shortfall); a page carries the `life` line — what it became, with its
      history. A decision draws as the `decide` scene: the draft, why it waits
      (*above the ₱20,000 you asked to see*), its receipt, and **Approve ·
      Change it · Look into it** — Approve the only accent (UI rule 5), from a
      loaded result (rule 8). **Done when:** "I need a purchase order system
      for Seikyo" → "make it 8 weeks, shops only" revises one object with v1
      reachable; "change it to 12 weeks" shows cover 56 → 84 and nothing else;
      keeping it adds a Needs-you row that opens as a figure with those three
      choices; the schedule is untouched, held by the existing version tests;
      Systems opens a system in the room. No eval — rendering over write
      proposals that already exist; the model's schema is unchanged.
- [ ] **P3.i he checks what happened** — the standard's last step (§9, §14:
      *check outcomes*, *results of those actions*), which no card did: every
      arc ended where Bob handed over a draft. Once P3.h lands arrivals, a
      kept Seikyo order gets a follow-up Bob raises himself — did the arrival
      match the order, how late, what was short — recorded against the order
      and said the next morning; an order with no arrival yet says so. No
      screen, no model change: a read over orders and transfers, a line in
      the morning. **Done when:** an order kept on Monday and a transfer
...
**Phase 5 — three decisions, not cards.** Asked 2026-09-19; the owner: *"dont
have an answer and dont need one"*. Written down so they are not forgotten,
decided when they matter, and no card is cut for any of them until then.
**(1) What "build it" means** — Bob assembling his existing reads into
systems with typed settings, which is what rules 1 and 4 allow today, or Bob
writing new tools himself, which they forbid; the standard (§11) reads as the
second. **(2) What Bob connects to** — email, calendar, accounting, messaging
(§16): none is planned; each becomes a source line when named. **(3) What
happens to the old Supabot app around him** — the dashboard, analytics and
legacy chat he sits inside today: the chrome he keeps, or the app he becomes
(§19). Also unmeasured and required: the room at phone width (UI rule 7) — no
frame has ever been taken under 1440.
...
- **S.6 people and permissions** (8) — who else uses Bob and what each
  may see.
- **S.7 the vending aisle feed** (17) — `vending_aisles` last has data from
  2026-08-05; the morning says so and covers nothing from it. AJI CMG's feed,
  restarted or re-exported.
- **S.8 expenses** (2) — nothing on expenses exists; "I looked at expenses
  too" in the design is authored. One export or one table.
- **S.9 customers, promotions, operational events** (2) — none exists; each
  is a source before it is a read.
- **S.10 the low-stock level in StoreHub** (10) — never set on any product,
  so "getting low" can never be flagged, only "hit zero". Set once, by you.
- **S.12 a StoreHub API key and its documentation** (8, P3.h) — asked for from StoreHub Customer Care
  by the account's master email; the API has a `/purchaseOrders` route and no transfer route, and publishes
  no reference. With the key and the document, purchase orders become a scheduled pull through
  `import_file`; transfers stay an upload.
- **S.11 the autumn of 2024** (the sales record) — six days between
  2024-09-01 and 2024-12-31; a StoreHub transaction export for those months,
...
- [x] **P6.e the canvas is the design** **BUILT 2026-09-20, locally.** The owner, over the design on his phone against P6.b's frame: "it doesn't look like this ... still kinda a thread." P6.b had been marked within measure of v2 and was not: every block on his arrangement still wore the packed board's three-sentence head, 72px hero, 34px air and head-chart-source shape. Laid out by him (`TileProps.canvas`), a block is now drawn to `ops/ideal/how-are-we-doing-v2.html`'s own numbers: `.q` one-line head (600 15px serif, run-on at 400), `.totals` 34/`.pair` 40/`.quiet` 22 with `this 30 days · was ₱…` beside and the thought under as the verdict, `.rhythm` 940×280 in ink with axis, `21 Aug` ticks, the note bracketed over a band coloured by its rows' direction, a legend in words; `.multiples` as one strip (no swatch, nothing dimmed, the `span`-named shop lit with the callout under it — `span` now takes one label); `.mv` drops from the right edge; `.need ul` unruled; 48px between sections, sections tight inside (`panel`). Relation lines are not drawn on the canvas. Packed board unchanged. Guards held it honest: palette (paint only), accentUse (no --up/--down in CSS), ink (no colour on quiet). Frames `verification/frames/canvas5/`. Still unverified live (P6.c).
- [x] **P6.f next, under the figures, at length** **BUILT 2026-09-20, locally.** The owner: "move what i'd do next to the right side, you can be more in depth on it, not just a few lines if needed." `ReadingNext` moved from the words column to the foot of the figures area (after the Board, with the foot offers), keeps his line breaks, marks figures with their read; `voice.reading.slots.next` max_length 320→1400 and its `about` asks for the steps, what each needs from the person, what to watch after; the prompt's NEXT clause says "under the figures ... at length" and the prompt sits at 1,800/1,800 (the depth rides on `compose`). `.r-next` 62ch, left rule (keptChrome updated). Seen in `verification/frames/canvas6/canvas-*-full.png`.
- [x] **P6.g build talks on the canvas** **BUILT 2026-09-20, locally.** The owner: "how about other types of talks? to test all the other features like building stuff. it must be ready for everything." Three scenes through the real room: `canvas-order` (draft the Seikyo order — the real `get_purchase_plan` rows for Seikyo SEK001, a `draft` block), `canvas-watch` (a watch on Greenhills' basket — the real basket-by-week read and a `system` block over the automations reader's row), `canvas-memory` (the recorded memory fixture). A built thing on the canvas opens with his claim and thought (`BuiltHead`), sits unboxed, and a `system` draws what a watch says about itself — condition, where, schedule, backtest — which `self_reader.py` now returns beside `what/state/by`. Not yet drawn: a saved workflow, a page edit, a standing question — same shapes, no fixture. Still unverified live (P6.c).
- [x] **P6.h one page** **BUILT 2026-09-21, locally.** The owner: "the rest are just kinda bad ... it should feel great in every scenario ... add what i would do next to the page so it feels like ONE PAGE ... still too much text on the left." Three things: (1) `{"next": true}` is a leaf of the arrangement — the Board draws the plan (`foot`) where he places it, else last, packed too; (2) the left is the headline and `voice.body.max_words` 40 (was 90) — every other paragraph is a `say` placed on the page, and the rewrite instruction says so; the body gate now runs AFTER the figure sweep so a figure with no receipt outranks length; (3) what the six pages needed: a callout under one named row on dumbbell/ranked/contributors (`span` of one label), a fold on the draft (twelve lines, the rest one tap), a line that fits its weight (280/200/140), the quiet label only on a quiet figure, `data-kind` on a figure. All seven scenes re-cut as pages: `verification/frames/canvas7/canvas-*-full.png`. Still unverified live (P6.c).
- [x] **P6.i the first live page** **2026-09-21, from the run record.** The owner's first live "how are we doing?" drew the packed machine board. Cause: the convergence cap refused `compose` (counted as a read) — fixed, with `test_a_compose_in_the_same_batch_as_a_refused_read_still_runs`; `this_week` compared at 00:25 on a Monday drew zeros — refused now (`min_closed_days`); and the turn was lost to the deploy of a752f41 landing mid-turn (no log row at all). See DOGFOOD_LOG 2026-09-21 00:25. Open: a deploy drains nothing; a standing question worded as a reminder is asked as a question.
- [x] **P6.j the second live page** **2026-09-21, from the run record.** He composed a real arrangement and the room threw it away: `_posts` never passed `arrangement` to `_answer_payload` (so no post has ever carried one) and `restore.ts` never read it — fixed both ends, held by `test_the_post_carries_the_page_he_laid_out` and restore.test.ts. Also: the packed grid overlaps while heights are unmeasured (now measured in the same layout pass); the canvas line drew at a fixed 940 viewBox in any column (now measures its box); and 156 s / 8 iterations — the body cap spent a round trip to rewrite 48 words (deterministic cut now) and three composes-only rounds named the claim without writing it (one reminder, `compose_without_answer`). His run as two scenes: `live-page`, `live-packed`.
- [x] **P6.k the page is the default** **BUILT 2026-09-21, locally.** The owner, of the board he reopened: "did we reach our goal?" No — it was the packing, because the canvas drew only when Bob sent an `arrangement`. Now a board HE composed is laid out as a page whether or not he said how (`pageOf`): his order, neighbours of the same shape paired, charts and lists full width, what he never wrote up after the page, the plan last. His own tree still wins. The packing draws what it draws in production — the machine's own board — and its tests say so. Two things the change exposed and fixed: a line lost its subject's swatch on the canvas (identity.dom.test.tsx), and the relation word ("why", "against that") is kept on a room-laid page wherever position cannot carry it, suppressed only on a page he laid out himself.
- [x] **P7 the page is a document** **BUILT 2026-09-21, locally — NOT YET SEEN FROM A LIVE TURN.** The owner: "i want it like an artifact claude can make … it feels like it has to fit the stuff in columns and rows or a grid but an artifact/page isnt like that. it makes its own … buttons, drop downs, filters … case to case." Target: `ops/ideal/the-page-bob-writes.html` (https://claude.ai/artifact/1ndAsibAefg4iHrdGdfcTf). The arrangement grew the parts of a page — `lede`, `head`, `say`, `note`, `{caveat: true}`, `tabs`, and on a block `beside` / `size` / `control` — and a sentence may hold a LIVE FIGURE by reference (`{key}`, `{key.change}`, `{key.was}`): the row's own value, tappable for its receipt, never a digit of his (rule 9 unchanged; a digit still drops the line). A figure is paired with the words that follow it, floats beside them, and balances itself (other size, then under them). His caveat, placed, is the margin note of the section it qualifies and leaves the left column — the left is the headline, forty words and the asks. One line from his mark to the page instead of one per figure. What counts as a block changed with it: a figure that lives in a sentence and a control carried on a figure are not counted against `max_blocks` (own bound `refs.max_in_words`), headings have their own bound, and on the client a control no longer REPLACES the chart it drives (`readIdentity`) nor do sentence-figures count against the twelve. Charts on the page: the point he called out wears its figure, a dot a day, a legend in the read's own two windows, a dumbbell scale that does not start at zero, facing lists on one scale, a list that drops what every row shares, the window control as a dropdown where its figure is narrow. Scene `doc-doing` is the 21 Sep turn written this way and PUT THROUGH `compose.compose`; frames in `verification/frames/p7-doc/`. Frontend 1165, backend 2162. **Open:** whether Bob writes such a page is P6.c's question and needs one live turn.
- [x] **P8 one voice on the page** **BUILT 2026-09-21 from his own live turn, locally.** The owner, of the first page Bob wrote himself: *"it still doesnt look like your artifact it still looks like a reskin."* He was right, and the run record (post f3a7afba, 15:05) says why: the document grammar worked — heads, paragraphs, a placed caveat, rows, one figure beside its words — but every block on it still announced itself with a `question` in bold serif, a `claim` and a `thought`, inside a section that already had a heading and a paragraph. Four voices per figure; an article has one. Three of the claims WERE the section head, word for word. So on a document a block draws its claim as a 13px caption and nothing else: no question (the head is the question), no thought (the paragraph beside it is the thought) unless the thought POINTS — a `span` on a chart, a callout on a named row — and no claim at all where `restated` says the head or the prose beside it already made it. The recipe was the other half and was telling him the opposite ("ON THE PAGE A HEAD IS ONE LINE: the bold lead is your `question`…", written for the board): it now says YOU ARE WRITING, NOT CAPTIONING — a lede that says the answer again with its figures inside it, two or three paragraphs a section, `**bold**` for a finding, no `size` unless the figure needs the width. Also from his page: his plan ran as one paragraph and drew a wall, so the page breaks it on his own openings ("Greenhills first.", "Magnolia second.") and changes not a word; the span band is a bracket, not a filled region; and `get_attention` drew a stock count as "₱0" and six rows wore the read's own rank — each row now draws by the `measure` it names, in its own unit, under the `section` it belongs to. Scene `doc-live` is his turn, from the record. Frontend 1180, backend 2162. **Open:** his next live turn is the test of the recipe half.
- [x] **P10 one page, and the page takes the room** **BUILT 2026-09-21, locally.** The owner, of the document drawn in the beside room: *"were getting there but theres still too many gaps, it still feels like its trying to fill in columns not the one big page"*, and, offered the two structural causes I could find, *"ok do both"*. **The page takes the room:** his side stops growing at 360 and the page takes the rest, capped at 900 — the artifact's own `.room` and `.page` — where it was 29fr:47fr of whatever was left after reserving 232px for a sidebar that was closed, which gave the page 703px and made every figure full width because nothing narrower left a measure prose reads at. The reservation goes too: opening the sidebar now narrows the room instead of sliding a composition sized for it. **One page, not a pane:** the room scrolls as one document and his side is sticky beside it, where the figures scrolled inside a fixed frame with two arrows and his words inside another. Both reverse decisions of his from 2026-09-17 and are recorded in DECISIONS. Also, from the same render: a figure with nothing beside it takes the page (a `wide` list alone drew at 56% with 400px of nothing beside it), a figure on a page is not capped at 760, a list is a block so its rows fill, and the pair that measures a figure against its words has no box of its own so the text after it wraps up the side. Frontend 1179. **Open:** the prose half is Bob's and lands on his next live turn.
- [x] **P12 he is told he writes a page** **BUILT 2026-09-21, locally.** The owner, reading the answers rather than the layout: *"the way it answers it still doesnt know it can generate pages."* It did not, and the prompt was why: every word in it about the surface was the board era's — *"The right of the screen is your reasoning: each block you compose is a STEP — the question it answered, your claim answering it, the figure"* — and *"THE BOARD CARRIES THE FIGURES; YOUR WORDS CARRY THE UNDERSTANDING"*. The page existed only in the `compose` tool's description, which he reads at the moment he composes, long after he has decided what to read and what to say. It now says the right of the screen is A PAGE YOU WRITE on `compose` — an opening sentence carrying the answer with its figures in it, headings that state what a section found, paragraphs, figures beside the words about them — and that the PAGE carries the figures and the reasoning while his words left are the conclusion. Written to fit: the prompt is 1,798/1,800, and "a block carries a claim and nothing else" stayed on the tool where the budget rule says it belongs. Backend 2167. **Open:** this and P8's recipe are both unverified live — one turn tests both.
- [x] **P13 nothing follows the plan** **BUILT 2026-09-21 from his 17:05 turn, locally.** The owner: *"wtf happened here its just all wrong"*. The page he wrote is the best prose he has produced — a lede with its figures in it, four heads, nine paragraphs with his own emphasis, his caveat placed — and he put ONE of his nine figures on it. The room drew the other eight AFTER the plan, which is the rule P6.h already made and which had only ever been applied to a plan the ROOM placed. Fixed: the leftovers are drawn where the plan is, above it. Two more from the same turn: he placed `movers-rose`, a key he had not composed, and the placement was dropped — a page given before the last compose silently loses the figure it was written around, so a place is KEPT now and drawn when its block arrives; and the coercion naming unplaced blocks said "'x' was not placed", which reads as a detail when the number is eight, so it says the count. The instruction he reads BEFORE composing now says EVERY BLOCK YOU COMPOSE GOES ON THE PAGE — the coercion arrives in the result of his last call and he never reads it. Also reverted: P8's run-on plan splitter, which broke his sentence in half ("a clean stock count." became step two); the page splits on his blank lines and guesses nothing. Frontend 1179, backend 2167.
- [x] **P14 the page is enforced, not asked for** **BUILT 2026-09-21.** Four rounds of instruction had failed, and `agent/prose.py` had already written down why: *"The 8,623-word prompt asked for it in three places and got 22%; the 1,793-word prompt asked once and got 19.5%. Words do not move it, so the loop enforces it."* So: **(a)** a notice the desk classifies `explains_only` is no longer `must_convey` — 23 kinds were BOTH, so the loop required in his prose exactly what UI rule 4 tells the surface never to draw, and appended it verbatim when he left it out; his page of 17:34 opened with 155 words of caveat whose longest clause was `comparison_incomplete`. Read from the one classification now, so the halves cannot disagree again; the notice still reaches the MODEL with its guidance. **(b)** the PAGE GATE: where he wrote a page as a page and left two or more of his own figures off it, the turn does not settle — one corrective round naming them, then it stands (`composition.arrangement.gate`, gap `page_left_blocks_off`). Measured against his real 17:05 turn: 5 left off. **(c)** the PLAN IS A LIST — `next` takes steps and stores them as the paragraphs the page splits on, because twice the page tried to find the steps inside a paragraph and twice it broke one of his sentences in half. **(d)** a caption is held to 0.45 against its own head (his read "Three shops fall, three hold, Rockwell gains" under "Three shops fall; the rest are fine"), the date line stopped counting reads, and a section that ENDS with one figure draws it at the top of its words — text can only sit beside a figure that comes before it, which is why no live page has ever had any. Frontend 1181, backend 2179. Speed baseline to beat: median 130 s / 9,444 output tokens today, against 37 s / 2,100 before the page existed.
- [x] **P16 which model answers is a switch, and where the time actually goes** **BUILT 2026-09-21.** The owner ran out of Anthropic credit mid-session: *"try this deepseek api for abit ... but make it easy to swap."* `agent/provider.py` is a switch, not a port — UNSET, the model, the ceiling and the client kwargs are the ones the loop has always used, because every measurement in this repo was taken on Anthropic; SET, it is a base URL, a model name and the NAME of the variable holding that key, so no key is named in the repo and `describe()` gives a log a hostname and a model (`urlsplit().hostname`) and nothing else. DeepSeek publishes an Anthropic-shaped endpoint and it was probed feature by feature against the exact request this loop sends — tools, streaming, the `system` list, `cache_control` at both TTLs, the top-level one, `thinking`, `output_config`, the beta header — all accepted, so there is no second request shape to keep working. One whole turn ran on `deepseek-chat`: 228.8 s, 34 calls, 9 blocks and every one of them placed, caveat 55 words where Anthropic's was 155, and cache hits ARE reported (470,144 read), so the prefix is not repriced every round. **Then the run record was read for why a turn takes 130 s, and it answers three questions at once.** (a) **It is not effort.** "how are the stores?" already runs at `medium` and takes 136 s; "how are we doing" at `high` takes 123 s. The per-turn effort card did its job and there is nothing left there — that question is closed. (b) **It is not the reads.** `iteration_ms` on every broad turn since the page landed puts 47-85 s in ONE model call — 83,005 ms of a 123 s turn — while all the reads together cost about 30 s. Output tokens went 4,855 before the page to 9,164-12,538 after it. The turn is slow because Bob EMITS more, not because he looks more. (c) **And the convergence cap was never going to catch it**, because since P2S.10 it counts DECISIONS: `get_change(store=X)` expands to seven reads and counts as one, so DeepSeek's 29 net reads were about six counted calls against a limit of twelve. Nor is a ceiling on real reads the instrument — six Opus turns have run 22 or more, and Opus itself fanned out per shop on "how are we doing" at 07:38 the same day, which is the investigation ladder localizing, not enumeration. **What was fixed, and it is the compose payload:** the `arrangement` tree is the largest single thing in a compose — 2,818 against 1,291 bytes of blocks at 07:38, 3,543 against 2,714 at 09:34, 4,722 against 1,867 on the DeepSeek turn — and most broad turns compose two or three times, re-emitting the whole tree each time (09:34 composed three). They re-emit it because the tool said *"Optional; leave it out and it is packed"* and **the loop has never done that**: it keeps the arrangement for the rest of the turn, "exactly as a block he does not mention stays". The two ends disagreed and the model believed the tool. The description now says the layout STANDS and a later call sends it only to change it; `arrangement_resent` records the bytes when one is re-sent unchanged, because nothing can refuse tokens after they are generated — instruction is the only instrument here, so the next session reads a number instead of guessing. Held by `test_arrangement_stands_contract` (the two ends must agree) and `test_provider_contract`. Backend 2193. **AND IT IS NOW THE MODEL BOB RUNS ON** — the owner, after the measurements: *"go with the flash as default."* `deepseek-chat` (an alias for `deepseek-v4-flash`) is the DEFAULT provider in code, not four Railway variables, because a variable that went missing would send Bob silently back to an account with no credit; `BOB_PROVIDER=anthropic` is the one variable back to claude-opus-5 and the request every older number was measured on. Broad turn: Opus 136 s / $1.010, flash 229 s / $0.082, pro 647 s / $0.289 — same wall-clock across the turns run (130 s vs 136 s) for a twenty-third of the money, and a lookup stayed a lookup (12 s, two calls, $0.003). A BLIND PANEL PREFERRED ITS WORK 5-3 (eight judges, four lenses, both reading orders, neither page naming its model; page design unanimous): it reads wider and decides more — it refused to ship a replenishment plan built on stock recorded below zero, which Opus filed as a qualification. `deepseek-v4-pro` is NOT the upgrade: five times slower, six times flash's price, and it tripped more gates including P14's page gate, live for the first time. The cost of the choice: DeepSeek does not carry a required notice in its prose, so the loop appends the *"(added automatically)"* block on 2 of 3 turns against Opus's 8 of 197 — **the figure to beat is 4%**. 142 tests build a client and none had a key, so conftest sets a key-shaped string when the variable is absent. Recorded in DECISIONS. Backend 2195. **Open:** the caveat rate is the next thing to fix and the owner has asked for it; whether the layout re-sends stop is the next live turn's to answer; and the 47-85 s compose is P15.b's — two structures, blocks and arrangement, are emitted for one page. Bob's WRITE path (pin, save, schedule) has still never run on DeepSeek: `set_standing_question` needs the writers the web route injects, which a local `loop.run` does not have.
- [ ] **P17.0 the cheap fixes: he stops being told to do what is never drawn** — from the 2026-09-21 speed investigation (DECISIONS, same date). A broad answer went ~30 s → ~125 s on 09-17/18, BEFORE the page, when c6226ee (a thought on every chart) and eeb7c7d (17 chart types) landed; compose grew 11,705 → 27,413 chars, 11 → 19 block fields. On DeepSeek 75–91% of output is THINKING. Four fixes, no change to how the page looks: **(a)** DeepSeek has no `medium` (it maps to high) and ignores the per-message effort marker, so it ran at high on everything — send effort TOP-LEVEL on DeepSeek, `low` for broad (measured today ~241 s → ~152 s mean, half the thinking, 3/3 good pages; a lean, not proof); **(b)** delete the text that contradicts itself and asks for undrawn fields — `voice.reading.path` ("drawn as a step — the question it answered, in bold"), the board addendum's `question`/`under` sentences, the compose docstring's "a thought: one or two sentences", and the unused `spec` (0 of 186 blocks) — he wrote ~6,500 chars of `question`/`thought` that day that no page drew; **(c)** the recipe asks for a `span` and forbids the `thought` a span needs, and the validator drops a span without one, so every span he is asked for is thrown away; **(d)** a round settles when a compose names the claim and carries a lede, rather than costing another round for words beside it (the first live page lost ~100 s to that). **Done when:** a broad answer is measured before and after on the same data, and the saving is reported whatever it is.
- [ ] **P17.a one read that already has the picture** — `get_overview`: every read his best broad answers make (estate total, each shop, the week by day, what rose and fell, the shelf, the warning list), run in code and returned COMPACT and RANKED, each finding as a one-line FACT written by code with its row and receipts beside it — the Tableau Pulse pattern ("LLMs summarize templated insights much more easily than metric facts directly"), and rule 9 holds because code writes every digit. A composite read like `get_change`, so rule 5 holds. Drill-down tools stay, so the breadth the blind panel rewarded is kept, computed once. Built from exactly what his best broad turns already read, never guessed. **Done when:** a fresh broad question is one read and two rounds, measured, with the page judged against today's.
- [ ] **P17.b sections: he writes the findings, code lays them out** — he emits, per section, a `head` that states the finding, one to three `say` lines and the figures it rests on; code derives kind (`default_composition.shape_for`, his override kept), size (`roomFor`), pairing (`pageOf`'s PAIRABLE) and caveat placement, and compiles the same arrangement the renderer already draws, so posts and the frontend do not change. One structure, not blocks plus a tree; "left a figure off the page" becomes impossible, so the page gate's round goes. Keys stay model-chosen for change-by-key and "this is that"; compiled blocks must not carry `default`. The derivation thresholds move into metrics.yaml. It is also the flat, self-contained shape that makes streaming the page possible later. **Done when:** a broad page is emitted as sections and draws within measure of today's, measured for time.
- [ ] **P15.a the page says what the artifact says** **TWO OF FOUR BUILT 2026-09-21.** A FOLD: a layout, not a leaf, so it costs the same `max_nodes`/`max_depth` as a row and what is inside is ordinary page; native `details`, keyboard-reachable, CLOSED AT REST so the page's first still frame is the short one. A fold with no label hides something behind the word "more", so it is laid out as a stack and said on `coerced`; its label carries no digit because the count beside it is the DRAWING's, off the children (rule 9). ONE MARGIN NOTE AT A TIME: a run of `note` leaves is one aside with a paragraph each, sharing the box and the rule, with no word dropped. And he is TOLD both exist, in the grammar he reads as he composes — what to fold (what completeness demands and attention does not) and what never to (the answer, and a caveat that says a figure may be wrong). Rendered beside the design at 1440, which found what the tests could not: a margin note ENDING its section has no words beside it, and floated at 31% it left a hole down the left the width of the page — the owner's own "too many gaps". At the tail it runs at the measure. Scene `doc-fold`; frontend 1207, backend 2216. **Still open:** two facing lists as ONE object (one caption and one receipt line over two halves — the shared scale is already there, the caption and receipt are not), and an ACTION on the page, which today attaches to a row so *"say the word and I'll draft that list"* is still a sentence with no button. — the four things its grammar still cannot express, each of which he wrote around in prose on a live page. A FOLD, so a long thing is present without being a wall (the artifact folds the ten zero-lines and the caveat; the page has no disclosure leaf, so everything is shown or absent). TWO FACING LISTS AS ONE OBJECT — one caption and one receipt line over two halves, where today a `row` of two blocks is two of everything. AN ACTION ON THE PAGE: an offer attaches to a ROW today, so *"say the word and I will draft that list"* died as a sentence with no button. And ONE MARGIN NOTE AT A TIME — two asides stacked turned the bottom of his page back into columns. **Done when:** each renders from a recorded turn beside `ops/ideal/the-page-bob-writes.html`, and the action still cannot send anything without his confirmation (level five).
- [ ] **P15.b the page is the answer** **HALF BUILT 2026-09-21; the rest needs one decision.** Mapped before it was built, and the map says the plain reading breaks the floor: SEVEN trust gates read the left column's `answer` string (restatement, grounding, misstated figure, enumerated remainder, volunteering, tool-leak, notices — architecture rule 9's enforcement surface), the notice gate would become a FALSE-POSITIVE machine rather than go quiet, `ConversationLog.posts` returns early on an empty answer so a page-only turn would store no answer post at all (no thread, no pin target), the room would go blank for the 47-85 s a compose takes because `part="claim"` is the only ungated child of `.r-words`, and `voice.body.max_words: 40` is the ONLY total bound on his prose. Recorded in DECISIONS with the file and line of each. **Shipped:** the half needing no decision — *nothing on screen repeats the headline*. Where the page's lede restates the headline the headline is not drawn and the page keeps it; the notices and his caveat stay above the figures (UI rule 4). The bar is measured on his two live turns (`HEADLINE_RESTATED_AT` 0.45: DeepSeek's repeat at 0.57 goes, Opus's genuinely different lede and headline both stay), never while busy, never with no page. Frontend 1195. **Still open:** his prose moving ONTO the page, and the board derived from it — blocked on one decision, whether the streamed `answer` survives and if not what those seven gates measure instead. — the structural one, and the owner said the old stuff can go. Today the left column owns a headline, a 40-word body and the asks, and the page owns an opening sentence: two places own the answer, which is why the same finding appears four times and why policing it never finishes. His prose moves ONTO the page — the opening sentence IS the answer — and the left keeps his mark, what was asked, and the asks. Second half: a figure is DECLARED IN THE PAGE where it sits and the board is derived from it, so a figure that is not on the page cannot exist and there is nothing to forget. Keys stay, so pins, "this is that" and change-by-key keep working. **Done when:** one live turn produces a page that says each thing once, and nothing on screen repeats the headline.
...
owner's ("it feels right" on the live build, Open empty five days). The
sources decide the rest. The readable copy of this plan, with every card's
prompt, is **Bob, The Build Plan** in §6.
...
with each other, across builds with `--since`.        # the weekly sweep
    ...
    .venv\Scripts\python.exe ops/frames.py --scenes vocab vocab2 vocab3 --out verification/frames/p2s3
    .venv\Scripts\python.exe ops/frames.py --scenes doing --voice --out verification/frames/p2s5  # the line listening
    cd frontend && npx tsc -b --force                           # NOT -p, NOT bare -b
    cd frontend && npm run build

**EVERY PUSH TO MAIN REDEPLOYS RAILWAY, DOCS-ONLY INCLUDED** (seen 2026-09-19: `1910f428` served after a notes commit; its swap is the likely cause of the owner's 404 that hour). A note-only commit rides the next real push; never push notes while the owner is using him.
...
and every number in this file that came from it is marked as such.
    ...
    .venv\Scripts\python.exe -m pytest tests/evals/test_voice_evals_v2.py -q          # full, 14 turns, $4.79 MEASURED at P2S.✓ 2026-09-18 (was 11 turns, $1.51 at P1.e)
...
(the link the platform returned on the 2026-09-18 republish, version 75 after P2S.✓; version 78 after P3.h, 2026-09-19). **Since
