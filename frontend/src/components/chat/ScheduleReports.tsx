...
export function ScheduleReportModal({ open, onClose, onSaved, initial, existing }: ScheduleReportModalProps) {
  ...
  const [schedule, setSchedule] = useState<ScheduleValue>({ frequency: 'daily', times: ['08:00'], daysOfWeek: [0], daysOfMonth: [1] });
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      setSchedule({ frequency: 'daily', times: ['08:00'], daysOfWeek: [new Date().getDay() === 0 ? 6 : new Date().getDay() - 1], daysOfMonth: [1] });
      setChatIds([]);
    }
