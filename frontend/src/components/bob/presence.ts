 ...
 *   "waiting for approval"    a fact about the QUEUE, not about Bob's
 ...
 * typed something they have not sent. That is real: Bob is being addressed.
...
export interface PresenceInput {
   ...
   * Bob is writing an answer AND there are rows on the way to the screen
...
const AT_REST = new Set<BobState>(['idle', 'error', 'complete']);

/** The mark's state. Same union the CSS classes are keyed on — no new hue. */
export function presenceState({ state, composer, figures = 0 }: PresenceInput): BobState {
...
export interface LiveActivity {
   ...
   * these to say what Bob has done so far, in the same vocabulary
 ...
 *
 * Empty when the newest turn is not Bob's, or is finished: once a turn is
 * done the answer is the narration, and a stopped turn narrates nothing.
 */
export function liveActivity(turns: BobTurn[]): LiveActivity {
  const none: LiveActivity = {
    running: [], lastResult: null, completed: [], thinking: '', toolResults: 0, figures: 0,
  };
  const last = turns[turns.length - 1];
  if (last?.role !== 'bob' || last.done || last.cancelled) return none;
