...
describe('presenceState', () => {
  it('is the stream state whenever a turn is in flight', () => {
    for (const state of ['thinking', 'running', 'answering'] as BobState[]) {
  ...
  it('listens when the person addresses the composer and Bob is at rest', () => {
  ...
  it('never builds out of a state that is not answering', () => {
    for (const state of ['thinking', 'running', 'complete', 'idle'] as BobState[]) {
...
function bob(extra: Partial<Extract<BobTurn, { role: 'bob' }>> = {}): BobTurn {
  return {
...
describe('liveActivity', () => {
  ...
  it('goes quiet once the turn is done or was stopped', () => {
    ...
    expect(liveActivity([bob({ toolCalls: calls, done: { conversation_id: 'c', iterations: 1, tool_calls: 1, status: 'ok', notice_forced: false, usage: { input: 0, output: 0, cache_read: 0 }, cache_hit: false } })]).running).toEqual([]);
    expect(liveActivity([bob({ toolCalls: calls, cancelled: true })]).running).toEqual([]);
  });
});
...
describe('liveActivity.figures', () => {
  ...
  it('is zero for a turn whose calls all failed, so nothing claims to be building', () => {
    const turn = bob({ toolCalls: [call(1, { error: 'refused' })] as never });
