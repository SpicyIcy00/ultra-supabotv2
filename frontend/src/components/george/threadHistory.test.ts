...
const chat: ChatDetail = {
  thread_id: T, title: 'Which categories?', turns: [
    ...
    { role: 'george', text: 'Confectionery.', at: '2026-09-07T09:00:05+08:00', thinking: null,
...
describe('a George post', () => {
  it('becomes a text-only turn with no tool calls', () => {
    ...
    expect(toHistory([turn])).toEqual([{ role: 'george', text: brief.body, tool_calls: [] }]);
  });
...
describe('the caller’s own chat', () => {
  it('supplies the calls behind its answers, and names its posts', () => {
    ...
    expect(turns.map((t) => t.role)).toEqual(['george', 'user', 'george']);
