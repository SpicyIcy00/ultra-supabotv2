 ...
 * George stopped existing whenever you looked elsewhere.
 *
 ...
 *
 * THE COMPOSER TELLS THE MARK. Whether the person is addressing George —
 * focused on the box, or holding an unsent draft — is a real fact, and the
 * presence model turns it into `listening` only while George is otherwise
 * at rest. The composer reports; presence.ts decides.
 */
import { useMemo, useState, type ReactNode } from 'react';
import { useGeorgeStream } from '../../hooks/useGeorgeStream';
import { GeorgeCtx, type GeorgeContext } from './georgeContext';
import { liveActivity, presenceState, type ComposerActivity } from './presence';

export function GeorgeStreamProvider({ children }: { children: ReactNode }) {
  const stream = useGeorgeStream();
  ...
  return <GeorgeCtx.Provider value={value}>{children}</GeorgeCtx.Provider>;
}
