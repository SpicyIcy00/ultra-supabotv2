 ...
 * mark until the answer arrived. Now the running read, the clock and his own
 * narration are; and all of it goes the moment his answer starts.
 */
...
describe('while he works', () => {
  it('says the read that is running, in the trail\'s words, with the clock', () => {
    const { container } = render(<Doing live turn={turn({ toolCalls: [{ seq: 1, tool: 'get_sales', arguments: {} }] })} />);
  ...
  it('says thinking once the reads have landed and nothing is running', () => {
    ...
    const { container } = render(<Doing live turn={turn({ toolCalls: [landed] })} />);
    expect(container.querySelector('.r-doing-line')?.textContent).toMatch(/thinking…/);
  });

  it('draws his narration, which nothing drew before', () => {
    const { container } = render(<Doing live turn={turn({ narration: 'Rockwell is down; let me look at the drivers.' })} />);
    expect(container.querySelector('.r-doing-said')?.textContent).toContain('let me look at the drivers');
  });

  it('is gone the moment his answer starts, and when he is done', () => {
    expect(render(<Doing live turn={turn({ text: 'The basket' })} />).container.firstChild).toBeNull();
    cleanup();
    expect(render(<Doing live={false} turn={turn({})} />).container.firstChild).toBeNull();
  });
});
